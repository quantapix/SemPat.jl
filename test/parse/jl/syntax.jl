...
module JL
    ...
    function parse(s; raise::Bool=true, depwarn::Bool=true, show=true)
        ...
        x0 = Expr(x)
        x1 = JLParse.remlineinfo!(Meta.parse(s; raise, depwarn))
...
function parseall(s)
    ...
    while pos <= lastindex(s)
        x, pos = Meta.parse(s, pos)
        push!(xs, x)
    end
    if length(xs) == 0; throw(ParseError("end of input"))
    elseif length(xs) == 1; xs[1]
    else Expr(:block, xs...)
    end
end
...
let
    ...
    for (ex1, ex2) in [("5 .≠ x", "5 .!= x"),
                       ("5 .≥ x", "5 .>= x"),
                       ("5 .≤ x", "5 .<= x")]
        ex1 = JL.parse(ex1); ex2 = JL.parse(ex2)
...
# issue #5997
@test_throws ParseError JL.parse(": x")
@test_throws ParseError JL.parse("""begin
  :
  x""")
@test_throws ParseError JL.parse("d[: 2]")

# issue #6770
@test_throws ParseError JL.parse("x.3")

# issue #8763
@test_throws ParseError JL.parse("sqrt(16)2")
@test_throws ParseError JL.parse("x' y")
@test_throws ParseError JL.parse("x 'y")
@test JL.parse("x'y") == Expr(:call, :*, Expr(Symbol("'"), :x), :y)

# issue #18851
# qpx @test JL.parse("-2[m]") == Expr(:call, :-, Expr(:ref, 2, :m))
# qpx @test JL.parse("+2[m]") == Expr(:call, :+, Expr(:ref, 2, :m))
@test JL.parse("!2[3]") == Expr(:call, :!, Expr(:ref, 2, 3))
# qpx @test JL.parse("-2{m}") == Expr(:call, :-, Expr(:curly, 2, :m))
# qpx @test JL.parse("+2{m}") == Expr(:call, :+, Expr(:curly, 2, :m))
@test JL.parse("-2(m)") == Expr(:call, :*, -2, :m)

# issue #8301
@test_throws ParseError JL.parse("&*s")

# issue #10677
@test_throws ParseError JL.parse("/1")
@test_throws ParseError JL.parse("/pi")
@test JL.parse("- = 2") == Expr(:(=), :(-), 2)
@test JL.parse("/ = 2") == Expr(:(=), :(/), 2)
@test_throws ParseError JL.parse("< : 2")
@test_throws ParseError JL.parse("+ : 2")
@test_throws ParseError JL.parse("< :2")
@test JL.parse("+ :2") == Expr(:call, :(+), QuoteNode(2))

# issue #10900
@test_throws ParseError JL.parse("+=")
@test_throws ParseError JL.parse(".")
@test_throws ParseError JL.parse("...")

# issue #10901
@test JL.parse("/([1], 1)[1]") == :(([1] / 1)[1])

# issue #10997
@test JL.parse(":(x.\$f[i])") == Expr(:quote,
                                   Expr(:ref,
                                        Expr(Symbol("."), :x,
                                             QuoteNode(Expr(:$, :f))),
                                        :i))
...
# issue #10994
@test JL.parse("1 + #= \0 =# 2") == :(1 + 2)

# issue #10910
@test JL.parse(":(using A)") == Expr(:quote, Expr(:using, Expr(:., :A)))
@test JL.parse(":(using A.b, B)") == Expr(:quote,
                                       Expr(:using,
                                            Expr(:., :A, :b),
                                            Expr(:., :B)))
@test JL.parse(":(using A: b, c.d)") == Expr(:quote,
                                          Expr(:using,
                                               Expr(:(:),
                                                    Expr(:., :A),
                                                    Expr(:., :b),
...
@test JL.parse(":(import A)") == Expr(:quote, Expr(:import, Expr(:., :A)))
@test JL.parse(":(import A.b, B)") == Expr(:quote,
                                        Expr(:import,
                                             Expr(:., :A, :b),
                                             Expr(:., :B)))
@test JL.parse(":(import A: b, c.d)") == Expr(:quote,
                                           Expr(:import,
                                                Expr(:(:),
                                                     Expr(:., :A),
                                                     Expr(:., :b),
...
# issue #11332
@test JL.parse("export \$(Symbol(\"A\"))") == :(export $(Expr(:$, :(Symbol("A")))))
@test JL.parse("export \$A") == :(export $(Expr(:$, :A)))
#= qpx
@test JL.parse("using \$a.\$b") == Expr(:using, Expr(:., Expr(:$, :a), Expr(:$, :b)))
@test JL.parse("using \$a.\$b, \$c") == Expr(:using,
                                               Expr(:., Expr(:$, :a), Expr(:$, :b)),
                                               Expr(:., Expr(:$, :c)))
@test JL.parse("using \$a: \$b, \$c.\$d") ==
    Expr(:using,
...
# blocks vs. tuples
@test JL.parse("()") == Expr(:tuple)
# qpx @test JL.parse("(;)") == Expr(:tuple, Expr(:parameters))
# qpx @test JL.parse("(;;)") == Expr(:block)
# qpx @test JL.parse("(;;;;)") == Expr(:block)
@test_throws ParseError JL.parse("(,)")
@test_throws ParseError JL.parse("(;,)")
@test_throws ParseError JL.parse("(,;)")
# TODO: would be nice to make these errors, but needed to parse e.g. `(x;y,)->x`
#@test_throws ParseError JL.parse("(1;2,)")
#@test_throws ParseError JL.parse("(1;2,;)")
#@test_throws ParseError JL.parse("(1;2,;3)")
@test JL.parse("(x;)") == Expr(:block, :x)
@test JL.parse("(;x)") == Expr(:tuple, Expr(:parameters, :x))
@test JL.parse("(;x,)") == Expr(:tuple, Expr(:parameters, :x))
@test JL.parse("(x,)") == Expr(:tuple, :x)
# qpx @test JL.parse("(x,;)") == Expr(:tuple, Expr(:parameters), :x)
# qpx @test JL.parse("(x;y)") == Expr(:block, :x, LineNumberNode(1,:none), :y)
# qpx @test JL.parse("(x...;)") == Expr(:tuple, Expr(:parameters), Expr(:(...), :x))
@test JL.parse("(;x...)") == Expr(:tuple, Expr(:parameters, Expr(:(...), :x)))
@test JL.parse("(x...;y)") == Expr(:tuple, Expr(:parameters, :y), Expr(:(...), :x))
# qpx @test JL.parse("(x;y...)") == Expr(:block, :x, LineNumberNode(1,:none), Expr(:(...), :y))
# qpx @test JL.parse("(x=1;y=2)") == Expr(:block, Expr(:(=), :x, 1), LineNumberNode(1,:none), Expr(:(=), :y, 2))
@test JL.parse("(x,;y)") == Expr(:tuple, Expr(:parameters, :y), :x)
@test JL.parse("(x,;y=1)") == Expr(:tuple, Expr(:parameters, Expr(:kw, :y, 1)), :x)
@test JL.parse("(x,a;y=1)") == Expr(:tuple, Expr(:parameters, Expr(:kw, :y, 1)), :x, :a)
@test JL.parse("(x,a;y=1,z=2)") == Expr(:tuple, Expr(:parameters, Expr(:kw,:y,1), Expr(:kw,:z,2)), :x, :a)
@test JL.parse("(a=1, b=2)") == Expr(:tuple, Expr(:(=), :a, 1), Expr(:(=), :b, 2))
@test_throws ParseError JL.parse("(1 2)") # issue #15248

@test JL.parse("f(x;)") == Expr(:call, :f, Expr(:parameters), :x)

@test JL.parse("1 == 2|>3") == Expr(:call, :(==), 1, Expr(:call, :(|>), 2, 3))

# issue #24153
@test JL.parse("a|>b|>c|>d") == JL.parse("((a|>b)|>c)|>d")
@test JL.parse("a<|b<|c<|d") == JL.parse("a<|(b<|(c<|d))")
...
# issue #12626
@test JL.parse("a .÷ 1") == Expr(:call, :.÷, :a, 1)
@test JL.parse("a .÷= 1") == Expr(:.÷=, :a, 1)
...
let p = JL.parse("try
    ...
    @test p.args[2] === false
    @test p.args[3].args[end] == JL.parse("b,c = t")
end

# pr #13078
@test JL.parse("a in b in c") == Expr(:comparison, :a, :in, :b, :in, :c)
@test JL.parse("a||b→c&&d") == Expr(:call, :→,
                                 Expr(Symbol("||"), :a, :b),
                                 Expr(Symbol("&&"), :c, :d))
...
#= qpx
@test "foo\nbar" == JL.parse("\"\"\"\r\nfoo\r\nbar\"\"\"") ==
    JL.parse("\"\"\"\nfoo\nbar\"\"\"") == JL.parse("\"\"\"\rfoo\rbar\"\"\"") ==
    JL.parse("\"foo\r\nbar\"") == JL.parse("\"foo\rbar\"") == JL.parse("\"foo\nbar\"")
=#
...
# issue #14977
#= qpx
@test JL.parse("x = 1", 1) == (:(x = 1), 6)
@test JL.parse("x = 1", 6) == (nothing, 6)
@test_throws BoundsError JL.parse("x = 1", 0)
@test_throws BoundsError JL.parse("x = 1", -1)
@test_throws BoundsError JL.parse("x = 1", 7)
=#
# issue #14683
@test_throws ParseError JL.parse("'\\A\"'")
@test JL.parse("'\"'") == JL.parse("'\\\"'") == '"' == "\""[1] == '\42'

# issue #24558
@test_throws ParseError JL.parse("'\\xff'")
@test_throws ParseError JL.parse("'\\x80'")
@test_throws ParseError JL.parse("'ab'")
@test '\u2200' == "\u2200"[1]

@test_throws ParseError JL.parse("f(2x for x=1:10, y")
...
# issue #9503
@test JL.parse("x<:y") == Expr(:(<:), :x, :y)
@test JL.parse("x>:y") == Expr(:(>:), :x, :y)
@test JL.parse("x<:y<:z").head === :comparison
@test JL.parse("x>:y<:z").head === :comparison
...
# qpx test_parseerror("0x0.1", "hex float literal must contain \"p\" or \"P\"")
# qpx test_parseerror("0x1.0p", "invalid numeric constant \"0x1.0\"")
...
# issue #15828
@test Meta.lower(Main, JL.parse("x...")) == Expr(:error, "\"...\" expression outside call")

# issue #15830
@test Meta.lower(Main, JL.parse("foo(y = (global x)) = y")) == Expr(:error, "misplaced \"global\" declaration")
...
# issue #15661
@test_throws ParseError JL.parse("function catch() end")
@test_throws ParseError JL.parse("function end() end")
@test_throws ParseError JL.parse("function finally() end")

# PR #16170
@test Meta.lower(Main, JL.parse("true(x) = x")) == Expr(:error, "invalid function name \"true\"")
@test Meta.lower(Main, JL.parse("false(x) = x")) == Expr(:error, "invalid function name \"false\"")
...
# issue #16672
#= qpx
@test count(isline, JL.parse("begin end").args) == 1
@test count(isline, JL.parse("begin; end").args) == 1
@test count(isline, JL.parse("begin; x+2; end").args) == 1
@test count(isline, JL.parse("begin; x+2; y+1; end").args) == 2
=#
...
@test_throws ParseError JL.parse("Int [1,2,3]")
@test_throws ParseError JL.parse("Int [x for x in 1:10]")
@test_throws ParseError JL.parse("foo (x) = x")
@test_throws ParseError JL.parse("foo {T<:Int}(x::T) = x")

@test_throws ParseError JL.parse("Foo .bar")

@test_throws ParseError JL.parse("import x .y")
@test_throws ParseError JL.parse("using x .y")

@test_throws ParseError JL.parse("--x")
@test_throws ParseError JL.parse("stagedfunction foo(x); end")

@test JL.parse("A=>B") == Expr(:call, :(=>), :A, :B)

@test JL.parse("{1,2,3}") == Expr(:braces, 1, 2, 3)
# qpx @test JL.parse("{1 2 3 4}") == Expr(:bracescat, Expr(:row, 1, 2, 3, 4))
# qpx @test JL.parse("{1 2; 3 4}") == Expr(:bracescat, Expr(:row, 1, 2), Expr(:row, 3, 4))
@test JL.parse("{x for x in 1:10}") == Expr(:braces, :(x for x in 1:10))
@test JL.parse("{x=>y for (x,y) in zip([1,2,3],[4,5,6])}") == Expr(:braces, :(x=>y for (x,y) in zip([1,2,3],[4,5,6])))
@test JL.parse("{:a=>1, :b=>2}") == Expr(:braces, Expr(:call, :(=>), QuoteNode(:a), 1),
                                    Expr(:call, :(=>), QuoteNode(:b), 2))

@test JL.parse("[a,b;c]")  == Expr(:vect, Expr(:parameters, :c), :a, :b)
@test JL.parse("[a,;c]")   == Expr(:vect, Expr(:parameters, :c), :a)
@test JL.parse("a[b,c;d]") == Expr(:ref, :a, Expr(:parameters, :d), :b, :c)
@test JL.parse("a[b,;d]")  == Expr(:ref, :a, Expr(:parameters, :d), :b)
@test_throws ParseError JL.parse("[a,;,b]")
@test JL.parse("{a,b;c}")  == Expr(:braces, Expr(:parameters, :c), :a, :b)
@test JL.parse("{a,;c}")   == Expr(:braces, Expr(:parameters, :c), :a)
@test JL.parse("a{b,c;d}") == Expr(:curly, :a, Expr(:parameters, :d), :b, :c)
@test JL.parse("a{b,;d}")  == Expr(:curly, :a, Expr(:parameters, :d), :b)

# this now is parsed as getindex(Pair{Any,Any}, ...)
@test_throws MethodError eval(JL.parse("(Any=>Any)[]"))
@test_throws MethodError eval(JL.parse("(Any=>Any)[:a=>1,:b=>2]"))
...
# PR #17393
for op in (:.==, :.&, :.|, :.≤)
  @test JL.parse("a $op b") == Expr(:call, op, :a, :b)
end
for op in (:.=, :.+=)
  @test JL.parse("a $op b") == Expr(op, :a, :b)
end
...
# issue #7272
@test Meta.lower(Main, JL.parse("let
            global x = 2
            local x = 1
...
@test Meta.lower(Main, JL.parse("let x
                local x = 1
            end")) == Expr(:error, "local \"x\" declared twice")

@test Meta.lower(Main, JL.parse("let x = 2
                local x = 1
            end")) == Expr(:error, "local \"x\" declared twice")
=#
# issue #23673
...
let ex = Meta.lower(Main, JL.parse("\"a\"=1"))
...
for (str, tag) in Dict("" => :none, "\"" => :string, "#=" => :comment, "'" => :char,
                     ...
                     "struct" => :other)
  # qpx @test Base.incomplete_tag(JL.parse(str, raise=false)) == tag
end
...
# issue 10046
for op in ["+", "-", "\$", "|", ".+", ".-", "*", ".*"]
  @test_throws ParseError JL.parse("$op in [+, -]")
end
...
# PR #15592
let str = "[1] [2]"
    @test_throws ParseError JL.parse(str)
end
...
# issue 18002
@test JL.parse("Foo{T} = Bar{T}") == Expr(:(=), Expr(:curly, :Foo, :T), Expr(:curly, :Bar, :T))

# don't insert push_loc for filename `none` at the top level
let ex = Meta.lower(Main, JL.parse("""
begin
    x = 1
end"""))
...
# issue #7669
#= qpx
@test JL.parse("@a(b=1, c=2)") == Expr(:macrocall, Symbol("@a"), LineNumberNode(1, :none), :(b=1), :(c=2))
=#
# issue #19685
let f = function (x; kw...)
            return (x, kw)
...
let ε=1, μ=2, x=3, î=4
    ...
    @test JL.parse("\u00b5") === JL.parse("\u03bc")
    @test µ == μ == 2
    # NFC normalization of identifiers:
    @test JL.parse("\u0069\u0302") === JL.parse("\u00ee")
    @test î == 4
    # latin vs greek ε (#14751)
    @test JL.parse("\u025B") === JL.parse("\u03B5")
    @test ɛ == ε == 1
end
...
@test JL.parse("ccall([1], 2)[3]") == Expr(:ref, Expr(:call, :ccall, Expr(:vect, 1), 2), 3)
@test JL.parse("ccall(a).member") == Expr(:., Expr(:call, :ccall, :a), QuoteNode(:member))
...
# issue #20541
@test JL.parse("[a .!b]") == Expr(:hcat, :a, Expr(:call, :.!, :b))
...
# issue #20000
#= qpx
@test JL.parse("@m(a; b=c)") == Expr(:macrocall, Symbol("@m"), LineNumberNode(1, :none),
...
# Issue #21225
let abstr = JL.parse("abstract type X end")
    @test JL.parse("abstract type X; end") == abstr
    @test JL.parse(string("abstract type X", ";"^5, " end")) == abstr
    @test JL.parse("abstract type X\nend") == abstr
    @test JL.parse(string("abstract type X", "\n"^5, "end")) == abstr
end
let prim = JL.parse("primitive type X 8 end")
    @test JL.parse("primitive type X 8; end") == prim
    @test JL.parse(string("primitive type X 8", ";"^5, " end")) == prim
    @test JL.parse("primitive type X 8\nend") == prim
    @test JL.parse(string("primitive type X 8", "\n"^5, "end")) == prim
end
...
# issue #21440
@test JL.parse("+(x::T,y::T) where {T} = 0") == JL.parse("(+)(x::T,y::T) where {T} = 0")
@test JL.parse("a::b::c") == Expr(:(::), Expr(:(::), :a, :b), :c)

# issue #21545
f21545(::Type{<: AbstractArray{T,N} where N}) where T = T
@test f21545(Array{Int8}) === Int8
@test JL.parse("<:{T} where T") == Expr(:where, Expr(:curly, :(<:), :T), :T)
# qpx @test JL.parse("<:(T) where T") == Expr(:where, Expr(:(<:), :T), :T)
@test JL.parse("<:{T}(T) where T") == Expr(:where, Expr(:call, Expr(:curly, :(<:), :T), :T), :T)
...
# issue #20575
@test_throws ParseError JL.parse("\"a\"x")
@test_throws ParseError JL.parse("\"a\"begin end")
@test_throws ParseError JL.parse("\"a\"begin end\"b\"")

# issue #16427
@test_throws ParseError JL.parse("for i=1:1 end(3)")
@test_throws ParseError JL.parse("begin end(3)")
@test_throws ParseError JL.parse("while false end(3)")
...
# operator suffixes
@test JL.parse("3 +̂ 4") == Expr(:call, :+̂, 3, 4)
@test JL.parse("3 +̂′ 4") == Expr(:call, :+̂′, 3, 4)
@test JL.parse("3 +⁽¹⁾ 4") == Expr(:call, :+⁽¹⁾, 3, 4)
@test JL.parse("3 +₍₀₎ 4") == Expr(:call, :+₍₀₎, 3, 4)
for bad in ('=', '$', ':', "||", "&&", "->", "<:")
    @test_throws ParseError JL.parse("3 $(bad)⁽¹⁾ 4")
end
@test Base.operator_precedence(:+̂) == Base.operator_precedence(:+)

@test JL.parse("(x)ᵀ") == Expr(:call, :*, :x, :ᵀ)
...
# issue #7314
@test JL.parse("local x, y = 1, 2") == Expr(:local, Expr(:(=),
                                                      Expr(:tuple, :x, :y),
                                                      Expr(:tuple, 1, 2)))

@test_throws ParseError JL.parse("[2for i=1:10]")
@test_throws ParseError JL.parse("[1 for i in 1:2for j in 2]")
@test_throws ParseError JL.parse("(1 for i in 1:2for j in 2)")
# issue #20441
@test_throws ParseError JL.parse("[x.2]")
@test_throws ParseError JL.parse("x.2")
@test JL.parse("[x;.2]") == Expr(:vcat, :x, 0.2)

# issue #22840
@test JL.parse("[:a :b]") == Expr(:hcat, QuoteNode(:a), QuoteNode(:b))

# issue #22868
@test_throws ParseError JL.parse("x@time 2")
@test_throws ParseError JL.parse("@ time")

# issue #7479
@test Meta.lower(Main, JL.parse("(true &&& false)")) == Expr(:error, "invalid syntax &false")
...
@test_throws ParseError JL.parse("a[end end]")
@test_throws ParseError JL.parse("a[end;end]")
#@test_throws ParseError JL.parse("a[end;]")  # this is difficult to fix
let a = rand(8), i = 3
...
# issue #18912
@test_throws ParseError JL.parse("(::)")
@test JL.parse(":(::)") == QuoteNode(Symbol("::"))
@test_throws ParseError JL.parse("f(::) = ::")
@test JL.parse("(::A)") == Expr(Symbol("::"), :A)
@test_throws ParseError JL.parse("(::, 1)")
@test_throws ParseError JL.parse("(1, ::)")

# issue #18650
#= qpx
let ex = JL.parse("maximum(@elapsed sleep(1) for k = 1:10)")
...
# issue #23519
@test JL.parse("@foo[1]") == JL.parse("@foo([1])")
@test JL.parse("@foo[1 2; 3 4]") == JL.parse("@foo([1 2; 3 4])")
# qpx @test JL.parse("@foo[1] + [2]") == JL.parse("@foo([1]) + [2]")
@test JL.parse("@foo [1] + [2]") == JL.parse("@foo([1] + [2])")
# qpx @test JL.parse("@Mdl.foo[1] + [2]") == JL.parse("@Mdl.foo([1]) + [2]")
@test JL.parse("@Mdl.foo [1] + [2]") == JL.parse("@Mdl.foo([1] + [2])")
...
@test JL.parse("1_2.3_4") == 12.34
@test_throws ParseError JL.parse("1._")
@test_throws ParseError JL.parse("1._5")
@test_throws ParseError JL.parse("1e.3")
@test_throws ParseError JL.parse("1e3.")
@test JL.parse("2e_1") == Expr(:call, :*, 2, :e_1)
# issue #17705
@test JL.parse("2e3_") == Expr(:call, :*, 2e3, :_)
@test JL.parse("2e-3_") == Expr(:call, :*, 2e-3, :_)
# qpx @test JL.parse("2e3_\"x\"") == Expr(:call, :*, 2e3, Expr(:macrocall, Symbol("@__str"), LineNumberNode(1, :none), "x"))

# misplaced top-level expressions
@test_throws ErrorException("syntax: \"\$\" expression outside quote") Core.eval(@__MODULE__, JL.parse("x->\$x"))
...
# qpx @test_throws ParseError("expected \"}\" or separator in arguments to \"{ }\"; got \"V)\"") JL.parse("f(x::V) where {V) = x")
# qpx @test_throws ParseError("expected \"]\" or separator in arguments to \"[ ]\"; got \"1)\"") JL.parse("[1)")
...
# issue #25391
@test JL.parse("0:-1, \"\"=>\"\"") == JL.parse("(0:-1, \"\"=>\"\")") ==
    Expr(:tuple, Expr(:call, :(:), 0, -1), Expr(:call, :(=>), "", ""))
# qpx @test JL.parse("a => b = c") == Expr(:(=), Expr(:call, :(=>), :a, :b), Expr(:block, LineNumberNode(1, :none), :c))
@test JL.parse("a = b => c") == Expr(:(=), :a, Expr(:call, :(=>), :b, :c))
...
# issue #25020
@test_throws ParseError JL.parse("using Colors()")
#= qpx
let ex = JL.parse("md\"x\"
...
@test JL.parse("-()^2")      == Expr(:call, :^, Expr(:call, :-), 2)
@test JL.parse("-(x,)^2")    == Expr(:call, :^, Expr(:call, :-, :x), 2)
# qpx @test JL.parse("-(x,;)^2")   == Expr(:call, :^, Expr(:call, :-, Expr(:parameters), :x), 2)
@test JL.parse("-(;x)^2")    == Expr(:call, :^, Expr(:call, :-, Expr(:parameters, :x)), 2)
@test JL.parse("-(x,y)^2")   == Expr(:call, :^, Expr(:call, :-, :x, :y), 2)
# qpx @test JL.parse("-(x...)^2")  == Expr(:call, :^, Expr(:call, :-, Expr(:(...), :x)), 2)
# qpx @test JL.parse("-(x...;)^2") == Expr(:call, :^, Expr(:call, :-, Expr(:parameters), Expr(:(...), :x)), 2)
# qpx @test JL.parse("-(x...;)")   == Expr(:call, :-, Expr(:parameters), Expr(:(...), :x))

# cases where parens are just grouping
@test JL.parse("-(x)^2")     == Expr(:call, :-, Expr(:call, :^, :x, 2))
@test JL.parse("-(a=1)^2")   == Expr(:call, :-, Expr(:call, :^, Expr(:(=), :a, 1), 2))
# qpx @test JL.parse("-(x;y)^2")   == Expr(:call, :-, Expr(:call, :^, Expr(:block, :x, LineNumberNode(1,:none), :y), 2))
# qpx @test JL.parse("-(;)^2")     == Expr(:call, :^, Expr(:call, :-, Expr(:parameters)), 2)
# qpx @test JL.parse("-(;;;;)^2")  == Expr(:call, :-, Expr(:call, :^, Expr(:block), 2))
@test JL.parse("-(x;;;)^2")  == Expr(:call, :-, Expr(:call, :^, Expr(:block, :x), 2))
@test JL.parse("+((1,2))")   == Expr(:call, :+, Expr(:tuple, 1, 2))

@test_throws ParseError("space before \"(\" not allowed in \"+ (\" at none:1") JL.parse("1 -+ (a=1, b=2)")
# issue #29781
@test_throws ParseError("space before \"(\" not allowed in \"sin. (\" at none:1") JL.parse("sin. (1)")
# Parser errors for disallowed space contain line numbers
@test_throws ParseError("space before \"[\" not allowed in \"f() [\" at none:2") JL.parse("\nf() [i]")
@test_throws ParseError("space before \"(\" not allowed in \"f() (\" at none:2") JL.parse("\nf() (i)")
@test_throws ParseError("space before \".\" not allowed in \"f() .\" at none:2") JL.parse("\nf() .i")
@test_throws ParseError("space before \"{\" not allowed in \"f() {\" at none:2") JL.parse("\nf() {i}")
@test_throws ParseError("space before \"m\" not allowed in \"@ m\" at none:2") JL.parse("\n@ m")
@test_throws ParseError("space before \".\" not allowed in \"a .\" at none:2") JL.parse("\nusing a .b")
@test_throws ParseError("space before \".\" not allowed in \"a .\" at none:2") JL.parse("\nusing a .b")
@test_throws ParseError("space before \"(\" not allowed in \"+ (\" at none:2") JL.parse("\n+ (x, y)")

@test JL.parse("1 -+(a=1, b=2)") == Expr(:call, :-, 1,
                                           Expr(:call, :+, Expr(:kw, :a, 1), Expr(:kw, :b, 2)))

# qpx @test JL.parse("-(2)(x)") == Expr(:call, :*, Expr(:call, :-,  2), :x)
@test JL.parse("-(x)y")   == Expr(:call, :*, Expr(:call, :-, :x), :y)
@test JL.parse("-(x,)y")  == Expr(:call, :*, Expr(:call, :-, :x), :y)
@test JL.parse("-(f)(x)") == Expr(:call, :-, Expr(:call, :f, :x))
# qpx @test JL.parse("-(2)(x)^2") == Expr(:call, :*, Expr(:call, :-, 2), Expr(:call, :^, :x, 2))
#= qpx
@test JL.parse("Y <- (x->true)(X)") ==
    Expr(:call, :<, :Y,
         Expr(:call, :-, Expr(:call, Expr(:->, :x, Expr(:block, LineNumberNode(1,:none), true)),
                              :X)))
=#
# issue #27641
@test JL.parse("√3x")   == Expr(:call, :*, Expr(:call, :√, 3), :x)
@test JL.parse("2^√3x") == Expr(:call, :^, 2, Expr(:call, :*, Expr(:call, :√, 3), :x))
@test JL.parse("√2^3")  == Expr(:call, :√, Expr(:call, :^, 2, 3))
@test JL.parse("-√2")   == Expr(:call, :-, Expr(:call, :√, 2))
@test JL.parse("√3x^2") == Expr(:call, :*, Expr(:call, :√, 3), Expr(:call, :^, :x, 2))
@test JL.parse("-3x^2") == Expr(:call, :*, -3, Expr(:call, :^, :x, 2))
@test_throws ParseError JL.parse("2!3")
@test_throws ParseError JL.parse("2√3")

# issue #27914
@test JL.parse("2f(x)")        == Expr(:call, :*, 2, Expr(:call, :f, :x))
@test JL.parse("f(x)g(x)")     == Expr(:call, :*, Expr(:call, :f, :x), Expr(:call, :g, :x))
# qpx @test JL.parse("2f(x)g(x)")    == Expr(:call, :*, 2, Expr(:call, :f, :x), Expr(:call, :g, :x))
# qpx @test JL.parse("f(x)g(x)h(x)") == Expr(:call, :*, Expr(:call, :f, :x), Expr(:call, :g, :x), Expr(:call, :h, :x))
@test JL.parse("2(x)")         == Expr(:call, :*, 2, :x)
# qpx @test JL.parse("2(x)y")        == Expr(:call, :*, 2, :x, :y)

@test_throws ParseError JL.parse("a.: b")
@test JL.parse("a.:end") == Expr(:., :a, QuoteNode(:end))
@test JL.parse("a.:catch") == Expr(:., :a, QuoteNode(:catch))
@test JL.parse("a.end") == Expr(:., :a, QuoteNode(:end))
@test JL.parse("a.catch") == Expr(:., :a, QuoteNode(:catch))
@test JL.parse("a.function") == Expr(:., :a, QuoteNode(:function))
...
@test_throws ParseError JL.parse("module module end")
...
# issue #26507
# qpx @test JL.parse("@try x") == Expr(:macrocall, Symbol("@try"), LineNumberNode(1,:none), :x)
# qpx @test JL.parse("@catch x") == Expr(:macrocall, Symbol("@catch"), LineNumberNode(1,:none), :x)
# qpx @test JL.parse("@\$x") == Expr(:macrocall, Symbol("@\$"), LineNumberNode(1,:none), :x)
...
@test_throws ParseError JL.parse("Any[:(end)]")

# issue #17781
#= qpx
let ex = Meta.lower(@__MODULE__, JL.parse("
    A = function (s, o...)
        f(a, b) do
        end
    end,
...
let ex = JL.parse("@test27521(2) do y; y; end")
    fex = Expr(:(->), Expr(:tuple, :y), Expr(:block, LineNumberNode(1,:none), :y))
    # qpx @test ex == Expr(:do, Expr(:macrocall, Symbol("@test27521"), LineNumberNode(1,:none), 2), fex)
    # qpx @test macroexpand(@__MODULE__, ex) == Expr(:tuple, fex, 2)
end
...
# #16356
# qpx @test_throws ParseError JL.parse("0xapi")

# #22523 #22712
@test_throws ParseError JL.parse("a?b:c")
@test_throws ParseError JL.parse("a ?b:c")
@test_throws ParseError JL.parse("a ? b:c")
@test_throws ParseError JL.parse("a ? b :c")
@test_throws ParseError JL.parse("?")
...
# #13079
@test JL.parse("1<<2*3") == :((1<<2)*3)

# #19987
@test_throws ParseError JL.parse("try ; catch f() ; end")
...
@test_throws ParseError JL.parse("x.'")
@test_throws ParseError JL.parse("0.+1")
...
# issue #28576
# qpx @test Meta.isexpr(JL.parse("1 == 2 ?"), :incomplete)
# qpx @test Meta.isexpr(JL.parse("1 == 2 ? 3 :"), :incomplete)
...
@test JL.parse("1…2") == Expr(:call, :…, 1, 2)
@test JL.parse("1⁝2") == Expr(:call, :⁝, 1, 2)
@test JL.parse("1..2") == Expr(:call, :.., 1, 2)
...
# issue #28506
#= qpx
@test Meta.isexpr(JL.parse("1,"), :incomplete)
@test Meta.isexpr(JL.parse("1, "), :incomplete)
@test Meta.isexpr(JL.parse("1,\n"), :incomplete)
@test Meta.isexpr(JL.parse("1, \n"), :incomplete)
=#
...
# issue #32626
@test JL.parse("'a'..'b'") == Expr(:call, :(..), 'a', 'b')
@test JL.parse(":a..:b") == Expr(:call, :(..), QuoteNode(:a), QuoteNode(:b))

# Non-standard identifiers (PR #32408)
@test JL.parse("var\"#\"") === Symbol("#")
@test JL.parse("var\"true\"") === Symbol("true")
@test JL.parse("var\"false\"") === Symbol("false")
@test_throws ParseError JL.parse("var\"#\"x") # Reject string macro-like suffix
@test_throws ParseError JL.parse("var \"#\"")
@test_throws ParseError JL.parse("var\"for\" i = 1:10; end")
# A few cases which would be ugly to deal with if var"#" were a string macro:
# qpx @test JL.parse("var\"#\".var\"a-b\"") == Expr(:., Symbol("#"), QuoteNode(Symbol("a-b")))
@test JL.parse("export var\"#\"") == Expr(:export, Symbol("#"))
@test Base.remove_linenums!(JL.parse("try a catch var\"#\" b end")) ==
      Expr(:try, Expr(:block, :a), Symbol("#"), Expr(:block, :b))
@test JL.parse("(var\"function\" = 1,)") == Expr(:tuple, Expr(:(=), Symbol("function"), 1))
# Non-standard identifiers require parens for string interpolation
@test JL.parse("\"\$var\\\"#\\\"\"") == Expr(:string, :var, "\"#\"")
@test JL.parse("\"\$(var\"#\")\"") == Expr(:string, Symbol("#"))
# Stream positioning after parsing var
# qpx @test JL.parse("var'", 1, greedy=false) == (:var, 4)

# quoted names in import (#33158)
# qpx @test JL.parse("import Base.:+") == :(import Base.+)
# qpx @test JL.parse("import Base.Foo.:(==).bar") == :(import Base.Foo.==.bar)
...
# issue #29982
@test JL.parse("'a'") == 'a'
@test JL.parse("'\U0061'") == 'a'
# qpx test_parseerror("''", "invalid empty character literal")
...
@test_throws Meta.ParseError JL.parse("[1 :\n2] == [1:2]")

# added ⟂ to operator precedence (#24404)
@test JL.parse("a ⟂ b ⟂ c") == Expr(:comparison, :a, :⟂, :b, :⟂, :c)
@test JL.parse("a ⟂ b ∥ c") == Expr(:comparison, :a, :⟂, :b, :∥, :c)

# only allow certain characters after interpolated vars (#25231)
# qpx @test JL.parse("\"\$x෴  \"",raise=false) == Expr(:error, "interpolated variable \$x ends with invalid character \"෴\"; use \"\$(x)\" instead.")
# qpx @test Base.incomplete_tag(JL.parse("\"\$foo", raise=false)) == :string

@testset "issue #30341" begin
  @test JL.parse("x .~ y") == Expr(:call, :.~, :x, :y)
  # Ensure dotting binary doesn't break dotting unary
  @test JL.parse(".~[1,2]") == Expr(:call, :.~, Expr(:vect, 1, 2))
end
...
@testset "operator precedence correctness" begin
  ...
  for f in ops, g in ops
      ...
      @test pf != pg
      expr = JL.parse("x$(f)y$(g)z")
      @test expr == JL.parse(pf > pg ? "(x$(f)y)$(g)z" : "x$(f)(y$(g)z)")
  end
end
...
@testset "issue #34650" begin
  for imprt in [:using, :import]
      @test Meta.isexpr(JL.parse("$imprt A, B"), imprt)
      @test Meta.isexpr(JL.parse("$imprt A: x, y, z"), imprt)
      ...
      ex = JL.parse("$imprt A, B: x, y", raise=false)
      # qpx @test ex == err

      ex = JL.parse("$imprt A: x, B: y", raise=false)
      # qpx @test ex == err
  end
end
...
# qpx @test JL.parse("aa\u200b_", raise=false) == Expr(:error, "invisible character \\u200b near column 3")
# qpx @test JL.parse("aa\UE0080", raise=false) == Expr(:error, "invalid character \"\Ue0080\" near column 3")
...
=#
# issue #36196
# qpx @test_throws ParseError("\"for\" at none:1 expected \"end\", got \")\"") JL.parse("(for i=1; println())")
# qpx @test_throws ParseError("\"try\" at none:1 expected \"end\", got \")\"") JL.parse("(try i=1; println())")
...
@test_throws ParseError("invalid operator \"<---\"") JL.parse("1<---2")
@test_throws ParseError("invalid operator \".<---\"") JL.parse("1 .<--- 2")
@test_throws ParseError("invalid operator \"--\"") JL.parse("a---b")
@test_throws ParseError("invalid operator \".--\"") JL.parse("a.---b")
...
# issue #37664
@test_throws ParseError("extra token \"b\" after end of expression") JL.parse("a b")
@test_throws ParseError("extra token \"b\" after end of expression") JL.parse("a#==#b")
@test_throws ParseError("extra token \"b\" after end of expression") JL.parse("a #==#b")
@test_throws ParseError("extra token \"b\" after end of expression") JL.parse("a#==# b")

@test_throws ParseError("extra token \"2\" after end of expression") JL.parse("1 2")
@test_throws ParseError("extra token \"2\" after end of expression") JL.parse("1#==#2")
@test_throws ParseError("extra token \"2\" after end of expression") JL.parse("1 #==#2")
@test_throws ParseError("extra token \"2\" after end of expression") JL.parse("1#==# 2")
...
# qpx @test JL.parse("for x in 1:10 g(x) end") == JL.parse("for#==#x#==#in#==#1:10#==#g(x)#==#end")
# qpx @test JL.parse("(f->f(1))() do x x+1 end") == JL.parse("(f->f(1))()#==#do#==#x#==#x+1#==#end")
# qpx @test JL.parse("while i < 10 i += 1 end") == JL.parse("while#==#i#==#<#==#10#==#i#==#+=#==#1#==#end")
@test JL.parse("begin x=1 end") == JL.parse("begin#==#x=1#==#end")
# qpx @test JL.parse("if x<y x+1 elseif y>0 y+1 else z end") == JL.parse("if#==#x<y#==#x+1#==#elseif#==#y>0#==#y+1#==#else#==#z#==#end")
@test JL.parse("function(x) x end") == JL.parse("function(x)#==#x#==#end")
# qpx @test JL.parse("a ? b : c") == JL.parse("a#==#?#==#b#==#:#==#c")
@test_throws ParseError("space before \"(\" not allowed in \"f (\" at none:1") begin
  JL.parse("f#==#(x)=x")
end
# qpx @test JL.parse("try f() catch e g() finally h() end") == JL.parse("try#==#f()#==#catch#==#e#==#g()#==#finally#==#h()#==#end")
# qpx @test JL.parse("@m a b") == JL.parse("@m#==#a#==#b")
...
# import ... as
@test_throws ParseError("invalid syntax \"using A as ...\"") JL.parse("using A as B")
@test_throws ParseError("invalid syntax \"using A.b as ...\"") JL.parse("using A.b as B")
@test_throws ParseError("invalid syntax \"using A.b as ...\"") JL.parse("using X, A.b as B")
@test_throws ParseError("invalid syntax \"import A as B:\"") JL.parse("import A as B: c")
@test_throws ParseError("invalid syntax \"import A.b as B:\"") JL.parse("import A.b as B: c")
...
@testset "unicode modifiers after '" begin
    @test JL.parse("a'ᵀ") == Expr(:call, Symbol("'ᵀ"), :a)
    @test JL.parse("a'⁻¹") == Expr(:call, Symbol("'⁻¹"), :a)
    @test JL.parse("a'ᵀb") == Expr(:call, :*, Expr(:call, Symbol("'ᵀ"), :a), :b)
    @test JL.parse("a'⁻¹b") == Expr(:call, :*, Expr(:call, Symbol("'⁻¹"), :a), :b)
end
=#
@testset "issue #37393" begin
    ...
    @test_throws Meta.ParseError JL.parse("""
        for i
            in 1:3
        end""")
end
...
# PR #37973
# qpx @test JL.parse("1¦2⌿3") == Expr(:call, :¦, 1, Expr(:call, :⌿, 2, 3))

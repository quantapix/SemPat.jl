...
export default class TypeScriptServiceClientHost extends Disposable {
  ...
  constructor(
    ...
    this.client = this._register(new TypeScriptServiceClient(context, onCaseInsenitiveFileSystem, services, allModeIds));
  ...
  private getAllModeIds(descriptions: LanguageDescription[], pluginManager: PluginManager) {
    const allModeIds = flatten([...descriptions.map((x) => x.modeIds), ...pluginManager.plugins.map((x) => x.languages)]);
    return allModeIds;
  }
  ...
  private async diagnosticsReceived(kind: DiagnosticKind, resource: vscode.Uri, diagnostics: Proto.Diagnostic[]): Promise<void> {
  ...
  private createMarkerDatas(diagnostics: Proto.Diagnostic[], source: string): (vscode.Diagnostic & { reportUnnecessary: any; reportDeprecated: any })[] {
  ...
  private getDiagnosticSeverity(diagnostic: Proto.Diagnostic): vscode.DiagnosticSeverity {
    if (this.reportStyleCheckAsWarnings && this.isStyleCheckDiagnostic(diagnostic.code) && diagnostic.category === PConst.DiagnosticCategory.error) {

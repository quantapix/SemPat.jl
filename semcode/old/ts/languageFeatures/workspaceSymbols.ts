...
class TypeScriptWorkspaceSymbolProvider implements vscode.WorkspaceSymbolProvider {
  ...
  public async provideWorkspaceSymbols(search: string, token: vscode.CancellationToken): Promise<vscode.SymbolInformation[]> {
    ...
    return response.body.filter((item) => item.containerName || item.kind !== 'alias').map((item) => this.toSymbolInformation(item));
  }

  private get searchAllOpenProjects() {
    return this.client.apiVersion.gte(API.v390) && vscode.workspace.getConfiguration('typescript').get('workspaceSymbols.scope', 'allOpenProjects') === 'allOpenProjects';
  }

...
class TagClosing extends Disposable {
  ...
  private onDidChangeTextDocument(document: vscode.TextDocument, changes: readonly vscode.TextDocumentContentChangeEvent[]) {
    ...
    const priorCharacter = lastChange.range.start.character > 0 ? document.getText(new vscode.Range(lastChange.range.start.translate({ characterDelta: -1 }), lastChange.range.start)) : '';
    if (priorCharacter === '>') {
      return;
    }
...
function requireActiveDocument(selector: vscode.DocumentSelector) {

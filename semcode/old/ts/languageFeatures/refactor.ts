  ...
  public async execute(args: DidApplyRefactoringCommand_Args): Promise<void> {
    ...
    if (renameLocation) {
      ...
      if (args.codeAction.document.uri.scheme !== fileSchemes.walkThroughSnippet) {
        await vscode.commands.executeCommand('editor.action.rename', [args.codeAction.document.uri, typeConverters.Position.fromLocation(renameLocation)]);
      }
    }
  }
}
...
class InlinedCodeAction extends vscode.CodeAction {
  ...
  private static getWorkspaceEditForRefactoring(client: ITypeScriptServiceClient, body: Proto.RefactorEditInfo): vscode.WorkspaceEdit {
...
class SelectCodeAction extends vscode.CodeAction {
  constructor(info: Proto.ApplicableRefactorInfo, document: vscode.TextDocument, rangeOrSelection: vscode.Range | vscode.Selection) {
...
class TypeScriptRefactorProvider implements vscode.CodeActionProvider<TsCodeAction> {
  ...
  public static readonly metadata: vscode.CodeActionProviderMetadata = {
    providedCodeActionKinds: [vscode.CodeActionKind.Refactor, ...allKnownCodeActionKinds.map((x) => x.kind)],
    documentation: [
      {
  ...
  public async resolveCodeAction(codeAction: TsCodeAction, token: vscode.CancellationToken): Promise<TsCodeAction> {
  ...
  private convertApplicableRefactors(body: Proto.ApplicableRefactorInfo[], document: vscode.TextDocument, rangeOrSelection: vscode.Range | vscode.Selection): TsCodeAction[] {

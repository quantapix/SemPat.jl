...
async function processPicker(processes: AttachItem[], name?: string): Promise<string> {
  return new Promise<string>(async (resolve, reject) => {
    const menu = qv.window.createQuickPick<AttachItem>();
    if (name) {
      menu.value = name;
    }
...
async function getGoProcesses(): Promise<AttachItem[]> {
  ...
  if (!goRuntimePath) {
    qv.window.showErrorMessage(`Failed to run "go version" as the "go" binary cannot be found in either GOROOT(${getCurrentGoRoot()}) or PATH(${envPath})`);
    return processes;
  }
  const args = ['version'];
...
async function runCommand(processCmd: ProcessListCommand): Promise<{ err: cp.ExecException; stdout: string; stderr: string }> {

...
export function goLiveErrorsEnabled() {
  ...
  const files = qv.workspace.getConfiguration('files', null);
  const autoSave = files['autoSave'];
  const autoSaveDelay = files['autoSaveDelay'];
  if (autoSave !== null && autoSave !== undefined && autoSave === 'afterDelay' && autoSaveDelay < liveErrorsConfig.delay * 1.5) {
    return false;
  }
...
export function parseLiveFile(e: qv.TextDocumentChangeEvent) {
...
async function processFile(e: qv.TextDocumentChangeEvent) {
  ...
  const p = cp.execFile(gotypeLive, args, { env }, (err, stdout, stderr) => {
    ...
    if (err) {
      ...
      stderr.split('\n').forEach((error) => {
        ...
        const canonicalFilePath = qv.Uri.file(file).toString();
        const range = new qv.Range(+line - 1, +column, +line - 1, +column);
        const diagnostic = new qv.Diagnostic(range, message, qv.DiagnosticSeverity.Error);
      ...
      diagnosticMap.forEach((diagnostics, file) => {
        buildDiagnosticCollection.set(qv.Uri.parse(file), diagnostics);
      });
    }
  });
  if (p.pid) {

...
import { buildLanguageServerConfig, getLocalGoplsVersion, languageServerIsRunning, serverOutputChannel } from '../../direct/src/features/server';
...
export const outputChannel = qv.window.createOutputChannel('Go');

export const diagnosticsStatusBarItem = qv.window.createStatusBarItem(qv.StatusBarAlignment.Left);

// statusbar item for switching the Go environment
export let goEnvStatusbarItem: qv.StatusBarItem;

let modulePath: string;
export const languageServerIcon = '$(zap)';
export const languageServerErrorIcon = '$(warning)';

export function updateGoStatusBar(editor: qv.TextEditor) {
...
export async function expandGoStatusBar() {
  const options = [{ label: 'Locate Configured Go Tools', description: 'display go env' }, { label: 'Choose Go Environment' }];
  ...
  qv.window.showQuickPick(options).then((item) => {
    if (item) {
      switch (item.label) {
        case 'Locate Configured Go Tools':
          qv.commands.executeCommand('go.locate.tools');
          break;
        case 'Choose Go Environment':
          qv.commands.executeCommand('go.environment.choose');
          break;
        ...
        case 'Install Go Language Server':
          qv.commands.executeCommand('go.tools.install', [allToolsInformation['gopls']]);
          break;
        case "Open 'go.mod'":
          const openPath = qv.Uri.file(item.description);
          qv.workspace.openTextDocument(openPath).then((doc) => {
            qv.window.showTextDocument(doc);
          });
          break;
      }
    }
...
export async function initGoStatusBar() {
  if (!goEnvStatusbarItem) {
    goEnvStatusbarItem = qv.window.createStatusBarItem(qv.StatusBarAlignment.Left, 50);
  }
...
let statusBarEntry: qv.StatusBarItem;
...
export function addGoStatus(message: string, command: string, tooltip?: string) {
  if (!statusBarEntry) {
    statusBarEntry = qv.window.createStatusBarItem(qv.StatusBarAlignment.Right, Number.MIN_VALUE);
  }

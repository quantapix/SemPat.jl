...
import { correctBinname, envPath, expandFilePathInOutput, fixDriveCasingInWindows, getBinPathWithPreferredGopathGoroot, getCurrentGoWorkspaceFromGOPATH, getInferredGopath } from '../utils/pathUtils';
...
export class GoDebugSession extends LoggingDebugSession {
  ...
  protected async disconnectRequestHelper(response: DebugProtocol.DisconnectResponse, args: DebugProtocol.DisconnectArguments): Promise<void> {
   ...
   */
  protected inferLocalPathInGoRootFromRemoteGoPackage(remotePathWithLocalSeparator: string, relativeRemotePath: string): string | undefined {
    ...
    const localGoRootImportPath = path.join(goroot, srcIndex >= 0 ? remotePathWithLocalSeparator.substr(srcIndex) : path.join('src', relativeRemotePath));
   ...
   */
  protected inferLocalPathInGoPathFromRemoteGoPackage(remotePathWithLocalSeparator: string, relativeRemotePath: string): string | undefined {
    ...
    const localGoPathImportPath = path.join(gopath, indexGoModCache >= 0 ? remotePathWithLocalSeparator.substr(indexGoModCache) : path.join('pkg', 'mod', relativeRemotePath));
    if (this.fileSystem.existsSync(localGoPathImportPath)) {
      return localGoPathImportPath;
    }

    // Scenario 2: The file is in a package in $GOPATH/src.
    const localGoPathSrcPath = path.join(gopath, 'src', relativeRemotePath.split(this.remotePathSeparator).join(this.localPathSeparator));
  ...
  protected toLocalPath(pathToConvert: string): string {
    ...
    if (!substituteRule) {
      ...
      if (gopath && indexGoModCache > 0) {
        return path.join(gopath, pathToConvert.substr(indexGoModCache).split(this.remotePathSeparator).join(this.localPathSeparator));
      }
    }
  ...
  protected async threadsRequest(response: DebugProtocol.ThreadsResponse): Promise<void> {
    ...
    this.delve.call<DebugGoroutine[] | ListGoroutinesOut>('ListGoroutines', [], (err, out) => {
      ...
      const threads = goroutines.map(
        (goroutine) => new Thread(goroutine.id, goroutine.userCurrentLoc.function ? goroutine.userCurrentLoc.function.name : goroutine.userCurrentLoc.file + '@' + goroutine.userCurrentLoc.line)
      );
      if (threads.length === 0) {
  ...
  protected async stackTraceRequest(response: DebugProtocol.StackTraceResponse, args: DebugProtocol.StackTraceArguments): Promise<void> {
    ...
    this.delve.call<DebugLocation[] | StacktraceOut>(this.delve.isApiV1 ? 'StacktraceGoroutine' : 'Stacktrace', [stackTraceIn], async (err, out) => {
      if (err) {
      ...
      let stackFrames = locations.map((location, frameId) => {
        ...
        return new StackFrame(
          ...
          location.file === '<autogenerated>' ? null : new Source(path.basename(location.file), this.toLocalPath(location.file)),
          location.line,
          0
        );
      });
    ...
    this.delve.call<DebugVariable[] | ListVarsOut>('ListLocalVars', this.delve.isApiV1 ? [listLocalVarsIn] : [{ scope: listLocalVarsIn, cfg: this.delve.loadConfig }], (err, out) => {
      ...
      this.delve.call<DebugVariable[] | ListFunctionArgsOut>(
        ...
        (listFunctionErr, outArgs) => {
          ...
          const vars = this.delve.isApiV1 ? <DebugVariable[]>outArgs : (<ListFunctionArgsOut>outArgs).Args;
          ...
          this.getPackageInfo(this.debugState).then((packageName) => {
            ...
            this.delve.call<DebugVariable[] | ListVarsOut>('ListPackageVars', this.delve.isApiV1 ? [filter] : [{ filter, cfg: this.delve.loadConfig }], (listPkgVarsErr, listPkgVarsOut) => {
              if (listPkgVarsErr) {
              ...
              const globals = this.delve.isApiV1 ? <DebugVariable[]>listPkgVarsOut : (<ListVarsOut>listPkgVarsOut).Variables;
              let initdoneIndex = -1;
  ...
  protected variablesRequest(response: DebugProtocol.VariablesResponse, args: DebugProtocol.VariablesArguments): void {
    ...
    const loadChildren = async (exp: string, v: DebugVariable) => {
      ...
      if ((v.kind === GoReflectKind.Struct && v.len > v.children.length) || (v.kind === GoReflectKind.Interface && v.children.length > 0 && v.children[0].onlyAddr === true)) {
  ...
  private initLaunchAttachRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments | AttachRequestArguments) {
    this.logLevel = args.trace === 'verbose' ? Logger.LogLevel.Verbose : args.trace === 'log' ? Logger.LogLevel.Log : Logger.LogLevel.Error;
  ...
  private async setBreakPoints(response: DebugProtocol.SetBreakpointsResponse, args: DebugProtocol.SetBreakpointsArguments): Promise<void> {
      ...
      .then(() => {
        ...
        return Promise.all(
          args.breakpoints.map((breakpoint) => {
            ...
            return this.delve.callPromise('CreateBreakpoint', [this.delve.isApiV1 ? breakpointIn : { Breakpoint: breakpointIn }]).then(null, async (err) => {
              ...
              if (err.toString().startsWith('Breakpoint exists at')) {
                ...
                if (!existingBreakpoints) {
                  try {
                    const listBreakpointsResponse = await this.delve.callPromise<ListBreakpointsOut | DebugBreakpoint[]>('ListBreakpoints', this.delve.isApiV1 ? [] : [{}]);
                    existingBreakpoints = this.delve.isApiV1 ? (listBreakpointsResponse as DebugBreakpoint[]) : (listBreakpointsResponse as ListBreakpointsOut).Breakpoints;
                  } catch (error) {
  ...
  private async getPackageInfo(debugState: DebuggerState): Promise<string | void> {
    ...
    const dir = path.dirname(this.delve.remotePath.length || this.delve.isRemoteDebugging ? this.toLocalPath(debugState.currentThread.file) : debugState.currentThread.file);
    if (this.packageInfo.has(dir)) {
      return Promise.resolve(this.packageInfo.get(dir));
    }
    return new Promise((resolve) => {
      execFile(getBinPathWithPreferredGopathGoroot('go', []), ['list', '-f', '{{.Name}} {{.ImportPath}}'], { cwd: dir, env: this.delve.dlvEnv }, (err, stdout, stderr) => {
  ...
  private shutdownProtocolServer(response: DebugProtocol.DisconnectResponse, args: DebugProtocol.DisconnectArguments): void {
  ...
  public async initializeRemotePackagesAndSources(delve: Delve): Promise<void> {
    ...
    try {
      ...
      if (!delve.isApiV1 && this.remotePackagesBuildInfo.length === 0) {
        const packagesBuildInfoResponse: ListPackagesBuildInfoOut = await delve.callPromise('ListPackagesBuildInfo', [{ IncludeFiles: true }]);
...
function queryGOROOT(cwd: any, env: any): Promise<string> {
  return new Promise<string>((resolve) => {
    execFile(getBinPathWithPreferredGopathGoroot('go', []), ['env', 'GOROOT'], { cwd, env }, (err, stdout, stderr) => {
      if (err) {
        return resolve('');
      }

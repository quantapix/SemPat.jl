...
import { envPath, fixDriveCasingInWindows, getBinPathWithPreferredGopathGorootWithExplanation, getCurrentGoRoot, getInferredGopath, resolveHomeDir } from './utils/pathUtils';
...
export function getCheckForToolsUpdatesConfig(gocfg: qv.WorkspaceConfiguration) {
...
export function byteOffsetAt(document: qv.TextDocument, position: qv.Position): number {
...
export function canonicalizeGOPATHPrefix(filename: string): string {
  ...
  for (const workspace of workspaces) {
    ...
    if (filenameLowercase.substring(0, workspace.length) === workspace.toLowerCase() && (!currentWorkspace || workspace.length > currentWorkspace.length)) {
...
export async function getGoVersion(goBinPath?: string): Promise<GoVersion | undefined> {
  ...
  const docUri = qv.window.activeTextEditor?.document.uri;
...
export function isGoPathSet(): boolean {
  if (!getCurrentGoPath()) {
    ...
    qv.window.showInformationMessage('Set GOPATH environment variable and restart VS Code or set GOPATH in Workspace settings', 'Set GOPATH in Workspace Settings').then((selected) => {
...
export function isPositionInString(document: qv.TextDocument, position: qv.Position): boolean {
...
export function getFileArchive(document: qv.TextDocument): string {
...
let currentGopath = '';
export function getCurrentGoPath(workspaceUri?: qv.Uri): string {
  const activeEditorUri = qv.window.activeTextEditor && qv.window.activeTextEditor.document.uri;
...
export function getExtensionCommands(): any[] {
  const pkgJSON = qv.extensions.getExtension(extensionId).packageJSON;
  if (!pkgJSON.contributes || !pkgJSON.contributes.commands) {
    return;
  }
  const extensionCommands: any[] = qv.extensions.getExtension(extensionId).packageJSON.contributes.commands.filter((x: any) => x.command !== 'go.show.commands');
...
export function resolvePath(inputPath: string, workspaceFolder?: string): string {
  ...
  if (!workspaceFolder && qv.workspace.workspaceFolders) {
    workspaceFolder = getWorkspaceFolderPath(qv.window.activeTextEditor && qv.window.activeTextEditor.document.uri);
  }
  ...
  return new Promise((resolve, reject) => {
    p = cp.execFile(cmd, args, { env, cwd }, (err, stdout, stderr) => {
      try {
        ...
        if (!atLeastSingleMatch && unexpectedOutput && qv.window.activeTextEditor) {
          outputChannel.appendLine(stderr);
          if (err) {
            ret.push({
              file: qv.window.activeTextEditor.document.fileName,
              line: 1,
              col: 1,
              msg: stderr,
              severity: 'error',
...
export function handleDiagnosticErrors(document: qv.TextDocument, errors: ICheckResult[], diagnosticCollection: qv.DiagnosticCollection, diagnosticSource?: string) {
  diagnosticCollection.clear();

  const diagnosticMap: Map<string, qv.Diagnostic[]> = new Map();

  const textDocumentMap: Map<string, qv.TextDocument> = new Map();
  if (document) {
    textDocumentMap.set(document.uri.toString(), document);
  }
  // Also add other open .go files known to vscode for fast lookup.
  qv.workspace.textDocuments.forEach((t) => {
  ...
  errors.forEach((error) => {
    const canonicalFile = qv.Uri.file(error.file).toString();
    ...
    const range = new qv.Range(error.line - 1, startColumn, error.line - 1, endColumn);
    const severity = mapSeverityToVSCodeSeverity(error.severity);
    const diagnostic = new qv.Diagnostic(range, error.msg, severity);
  ...
  diagnosticMap.forEach((newDiagnostics, file) => {
    const fileUri = qv.Uri.parse(file);
 ...
 */
export function removeDuplicateDiagnostics(collection: qv.DiagnosticCollection, fileUri: qv.Uri, newDiagnostics: qv.Diagnostic[]) {
 ...
 */
function deDupeDiagnostics(buildDiagnostics: qv.Diagnostic[], otherDiagnostics: qv.Diagnostic[]): qv.Diagnostic[] {
...
function mapSeverityToVSCodeSeverity(sev: string): qv.DiagnosticSeverity {
  switch (sev) {
    case 'error':
...
export function getWorkspaceFolderPath(fileUri?: qv.Uri): string | undefined {
  if (fileUri) {
    const workspace = qv.workspace.getWorkspaceFolder(fileUri);
    if (workspace) {
 ...
 */
export function runGodoc(cwd: string, packagePath: string, receiver: string, symbol: string, token: qv.CancellationToken) {
  if (!packagePath) {
 ...
 * @param position
 */
export function isPositionInComment(document: qv.TextDocument, position: qv.Position): boolean {
  const lineText = document.lineAt(position.line).text;
  const commentIndex = lineText.indexOf('//');

  if (commentIndex >= 0 && position.character > commentIndex) {
    const commentPosition = new qv.Position(position.line, commentIndex);

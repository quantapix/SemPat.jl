...
const SECURITY_SENSITIVE_CONFIG: string[] = ['alternateTools', 'gopath', 'goroot', 'inferGopath', 'toolsGopath', 'toolsEnvVars'];
...
export async function initConfig(ctx: qv.ExtensionContext) {
  ...
  ctx.subscriptions.push(qv.commands.registerCommand('go.workspace.isTrusted.toggle', toggleWorkspaceIsTrusted));

  if (isTrusted) {
    return;
  }
  const ignored = ignoredWorkspaceConfig(qv.workspace.getConfiguration('go'), SECURITY_SENSITIVE_CONFIG);
  if (ignored.length === 0) {
    return;
  }
  const ignoredSettings = ignored.map((x) => `"go.${x}"`).join(',');
  const val = await qv.window.showWarningMessage(
  ...
  switch (val) {
    case 'Trust This Workspace':
      ...
      break;
    case 'More Info':
      qv.env.openExternal(qv.Uri.parse('https://github.com/golang/vscode-go/blob/master/docs/settings.md#security'));
      break;
    default:
      break;
  }
...
function ignoredWorkspaceConfig(cfg: qv.WorkspaceConfiguration, keys: string[]) {
...
export class Configuration {
  constructor(private _workspaceIsTrusted = false, private getConfiguration = qv.workspace.getConfiguration) {}
  ...
  // returns a Proxied qv.WorkspaceConfiguration, which prevents
  // from using the workspace configuration if the workspace is untrusted.
  public get(section: string, uri?: qv.Uri): qv.WorkspaceConfiguration {
...
// wrappedConfiguration wraps qv.WorkspaceConfiguration.
class WrappedConfiguration implements qv.WorkspaceConfiguration {
  constructor(private readonly _wrapped: qv.WorkspaceConfiguration) {
  ...
  public update(section: string, value: any, configurationTarget?: boolean | qv.ConfigurationTarget, overrideInLanguage?: boolean): Thenable<void> {
...
export const getGoConfig = (uri?: qv.Uri) => {
  return getConfig('go', uri);
};

// getGoplsConfig returns the user's gopls configuration.
export function getGoplsConfig(uri?: qv.Uri) {
  return getConfig('gopls', uri);
}

function getConfig(section: string, uri?: qv.Uri) {
  if (!uri) {
    if (qv.window.activeTextEditor) {
      uri = qv.window.activeTextEditor.document.uri;
    } else {
      uri = null;
    }
  }

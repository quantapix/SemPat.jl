...
import { containsTool, getConfiguredTools, getImportPath, getImportPathWithVersion, getTool, hasModSuffix, Tool, ToolAtVersion } from './goTools';
...
export async function installTool(tool: ToolAtVersion, goVersion: GoVersion, envForTools: NodeJS.Dict<string>, modulesOn: boolean): Promise<string> {
...
export async function promptForMissingTool(toolName: string) {
  ...
  if (tool.minimumGoVersion && goVersion.lt(tool.minimumGoVersion.format())) {
    qv.window.showInformationMessage(`You are using go${goVersion.format()}, but ${tool.name} requires at least go${tool.minimumGoVersion.format()}.`);
    return;
  }
  if (tool.maximumGoVersion && goVersion.gt(tool.maximumGoVersion.format())) {
    qv.window.showInformationMessage(`You are using go${goVersion.format()}, but ${tool.name} only supports go${tool.maximumGoVersion.format()} and below.`);
    return;
  }
...
Run "go get -v ${getImportPath(tool, goVersion)}" to install.`;
  const selected = await qv.window.showErrorMessage(msg, ...installOptions);
  switch (selected) {
    case 'Install':
...
export async function promptForUpdatingTool(toolName: string, newVersion?: semver.SemVer, crashed?: boolean, message?: string) {
  const tool = getTool(toolName);
  ...
  while (choices.length > 0) {
    const selected = await qv.window.showInformationMessage(updateMsg, ...choices);
    switch (selected) {
      case 'Always Update':
      ...
      case 'Release Notes':
        ...
        qv.commands.executeCommand('qv.open', qv.Uri.parse(`https://github.com/golang/tools/releases/tag/${tool.name}/v${newVersion}`));
        break;
      default:
        choices = [];
    ...
    cp.execFile(
      ...
      (err, stdout, stderr) => {
        if (err) {
          ...
          qv.window.showErrorMessage(`Failed to run '${goRuntimePath} env. The config change may not be applied correctly.`);
          return reject();
        }
        if (stderr) {
...
export async function offerToInstallTools() {
  ...
  if (missing.length > 0) {
    ...
    qv.commands.registerCommand('go.promptforinstall', () => {
      const installItem = {
        title: 'Install',
        async command() {
          removeGoStatus();
      ...
      qv.window.showInformationMessage('Failed to find some of the Go analysis tools. Would you like to install them?', installItem, showItem).then((selection) => {
        if (selection) {
          selection.command();
        } else {
          removeGoStatus();
...
async function suggestDownloadGo() {
  ...
  if (suggestedDownloadGo) {
    qv.window.showErrorMessage(msg);
    return;
  }

  const choice = await qv.window.showErrorMessage(msg, 'Go to Download Page');
  if (choice === 'Go to Download Page') {
    qv.env.openExternal(qv.Uri.parse('https://golang.org/dl/'));
  }
...
export async function latestToolVersion(tool: Tool, includePrerelease?: boolean): Promise<semver.SemVer | null> {
  ...
  try {
    ...
    const { stdout } = await execFile(goCmd, ['list', '-m', '--versions', '-json', `${tool.modulePath}@${version}`], { env, cwd: tmpDir });
...
async function defaultInspectGoToolVersion(binPath: string): Promise<{ goVersion?: string; moduleVersion?: string }> {
  ...
  try {
    ...
    outputChannel.appendLine(`Failed to determine the version of ${binPath}. For debugging, run "go version -m ${binPath}"`);

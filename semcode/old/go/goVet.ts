...
import { getGoVersion, getWorkspaceFolderPath, handleDiagnosticErrors, ICheckResult, resolvePath, runTool } from './util';
...
export function vetCode(vetWorkspace?: boolean) {
  ...
  if (!editor && !vetWorkspace) {
    qv.window.showInformationMessage('No editor is active, cannot find current package to vet');
    return;
  }
  if (editor.document.languageId !== 'go' && !vetWorkspace) {
    qv.window.showInformationMessage('File in the active editor is not a Go file, cannot find current package to vet');
    return;
  }
 ...
 */
export async function goVet(fileUri: qv.Uri, goConfig: qv.WorkspaceConfiguration, vetWorkspace?: boolean): Promise<ICheckResult[]> {
  epoch++;

...
interface Highlight {
  top: qv.TextEditorDecorationType;
  mid: qv.TextEditorDecorationType;
  bot: qv.TextEditorDecorationType;
  all: qv.TextEditorDecorationType;
}

let decorators: {
  type: 'highlight' | 'gutter';
  coveredGutter: qv.TextEditorDecorationType;
  uncoveredGutter: qv.TextEditorDecorationType;
 ...
 */
export function initCoverageDecorators(ctx: qv.ExtensionContext) {
...
export function updateCodeCoverageDecorators(coverageDecoratorConfig: any) {
  ...
  if (typeof coverageDecoratorConfig !== 'object') {
    qv.window.showWarningMessage("invalid go.coverageDecorator type, expected an 'object'");
  } else {
    for (const k in coverageDecoratorConfig) {
      if (coverageDecoratorConfig.hasOwnProperty(k)) {
        ...
        qv.window.showWarningMessage(`invalid coverage parameter ${k}`);
      }
    }
  }
  setDecorators();
  qv.window.visibleTextEditors.forEach(applyCodeCoverage);
}
...
function setDecorators() {
  ...
  decorators = {
    ...
    coveredGutter: qv.window.createTextEditorDecorationType({
      gutterIconPath: gutterSvgs[decoratorConfig.coveredGutterStyle],
    }),
    uncoveredGutter: qv.window.createTextEditorDecorationType({
      gutterIconPath: gutterSvgs[decoratorConfig.uncoveredGutterStyle],
    }),
    coveredHighlight: {
      all: qv.window.createTextEditorDecorationType(cone),
      top: qv.window.createTextEditorDecorationType(ctop),
      mid: qv.window.createTextEditorDecorationType(cmid),
      bot: qv.window.createTextEditorDecorationType(cbot),
    },
    uncoveredHighlight: {
      all: qv.window.createTextEditorDecorationType(uone),
      top: qv.window.createTextEditorDecorationType(utop),
      mid: qv.window.createTextEditorDecorationType(umid),
      bot: qv.window.createTextEditorDecorationType(ubot),
    },
  };
}
...
interface CoverageData {
  uncoveredOptions: qv.DecorationOptions[];
  coveredOptions: qv.DecorationOptions[];
}
 ...
 * @param editor
 */
export function applyCodeCoverage(editor: qv.TextEditor) {
...
function detailed(editor: qv.TextEditor, h: Highlight, opts: qv.DecorationOptions[]) {
  const tops: qv.DecorationOptions[] = [];
  const mids: qv.DecorationOptions[] = [];
  const bots: qv.DecorationOptions[] = [];
  const alls: qv.DecorationOptions[] = [];
  opts.forEach((opt) => {
    const r = opt.range;
    ...
    for (let line = r.start.line; line <= r.end.line; line++) {
      if (line === r.start.line) {
        const use: qv.DecorationOptions = {
          range: editor.document.validateRange(new qv.Range(line, r.start.character, line, Number.MAX_SAFE_INTEGER)),
        ...
        const use = {
          range: editor.document.validateRange(new qv.Range(line, 0, line, Number.MAX_SAFE_INTEGER)),
          hoverMessage: opt.hoverMessage,
        };
        mids.push(use);
      } else {
        const use = {
          range: new qv.Range(line, 0, line, r.end.character),
          hoverMessage: opt.hoverMessage,
        };
        bots.push(use);
      }
 ...
 */
export function removeCodeCoverageOnFileSave(e: qv.TextDocument) {
  if (e.languageId !== 'go' || !isCoverageApplied) {
    return;
  }

  if (qv.window.visibleTextEditors.every((editor) => editor.document !== e)) {
    return;
  }
 ...
 */
export function trackCodeCoverageRemovalOnFileChange(e: qv.TextDocumentChangeEvent) {
  if (e.document.languageId !== 'go' || !e.contentChanges.length || !isCoverageApplied) {
    return;
  }

  if (qv.window.visibleTextEditors.every((editor) => editor.document !== e.document)) {
    return;
  }
...
export function isPartOfComment(e: qv.TextDocumentChangeEvent): boolean {

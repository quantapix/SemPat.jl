...
const testOutputChannel = qv.window.createOutputChannel('Go Tests');
...
export function getTestEnvVars(config: qv.WorkspaceConfiguration): any {
...
export function getTestFlags(goConfig: qv.WorkspaceConfiguration, args?: any): string[] {
...
export function getTestTags(goConfig: qv.WorkspaceConfiguration): string {
 ...
 */
export async function getTestFunctions(doc: qv.TextDocument, token: qv.CancellationToken): Promise<qv.DocumentSymbol[] | undefined> {
  ...
  const testify = children.some((sym) => sym.kind === qv.SymbolKind.Namespace && sym.name === '"github.com/stretchr/testify/suite"');
  return children.filter((sym) => sym.kind === qv.SymbolKind.Function && (testFuncRegex.test(sym.name) || (testify && testMethodRegex.test(sym.name))));
}
 ...
 */
export function getTestFunctionDebugArgs(document: qv.TextDocument, testFunctionName: string, testFunctions: qv.DocumentSymbol[]): string[] {
 ...
 */
export function findAllTestSuiteRuns(doc: qv.TextDocument, allTests: qv.DocumentSymbol[]): qv.DocumentSymbol[] {
 ...
 */
export async function getBenchmarkFunctions(doc: qv.TextDocument, token: qv.CancellationToken): Promise<qv.DocumentSymbol[] | undefined> {
  ...
  const children = symbol.children;
  return children.filter((sym) => sym.kind === qv.SymbolKind.Function && benchmarkRegex.test(sym.name));
}
...
export async function goTest(testconfig: TestConfig): Promise<boolean> {
  ...
  if (!goRuntimePath) {
    qv.window.showErrorMessage(`Failed to run "go test" as the "go" binary cannot be found in either GOROOT(${getCurrentGoRoot()}) or PATH(${envPath})`);
...
async function getTestTargetPackages(testconfig: TestConfig, outputChannel: qv.OutputChannel) {
  ...
  if (testconfig.isMod) {
    ...
    getCurrentPackagePromise = Promise.resolve(currentGoWorkspace ? testconfig.dir.substr(currentGoWorkspace.length + 1) : '');
  ...
  if (testconfig.applyCodeCoverage) {
    ...
    switch (coverMode) {
      ...
      case 'atomic':
        ...
        break;
      default:
        qv.window.showWarningMessage(`go.coverMode=${coverMode} is illegal. Use 'set', 'count', 'atomic', or 'default'.`);
    }
  }
...
function processTestResultLineInJSONMode(pkgMap: Map<string, string>, currentGoWorkspace: string, outputChannel: qv.OutputChannel) {
  return (line: string) => {
    try {
...
function processTestResultLineInStandardMode(pkgMap: Map<string, string>, currentGoWorkspace: string, testResultLines: string[], outputChannel: qv.OutputChannel) {

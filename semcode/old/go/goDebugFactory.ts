...
export class GoDebugAdapterDescriptorFactory implements qv.DebugAdapterDescriptorFactory {
  ...
  public async createDebugAdapterDescriptor(session: qv.DebugSession, executable: qv.DebugAdapterExecutable | undefined): Promise<qv.ProviderResult<qv.DebugAdapterDescriptor>> {
  ...
  private async createDebugAdapterDescriptorDlvDap(configuration: qv.DebugConfiguration): Promise<qv.ProviderResult<qv.DebugAdapterDescriptor>> {
    ...
    return new qv.DebugAdapterServer(port, host);
  }
...
export async function startDapServer(configuration: DebugConfiguration): Promise<{ port: number; host: string; dlvDapServer?: ChildProcessWithoutNullStreams }> {
...
async function spawnDlvDapServerProcess(launchArgs: DebugConfiguration): Promise<ChildProcess> {
  ...
  if (!fs.existsSync(dlvPath)) {
    appendToDebugConsole(`Couldn't find dlv at the Go tools path, ${process.env['GOPATH']}${env['GOPATH'] ? ', ' + env['GOPATH'] : ''} or ${envPath}`);
  ...
  return await new Promise<ChildProcess>((resolve, reject) => {
    ...
    let started = false;
    const timeoutToken: NodeJS.Timer = setTimeout(() => reject(new Error('timed out while waiting for DAP server to start')), 5_000);
    ...
    p.stdout.on('data', (chunk) => {
      if (!started) {
        if (chunk.toString().startsWith('DAP server listening at:')) {
          ...
          stopWaitingForServerToStart(`Expected 'DAP server listening at:' from debug adapter got '${chunk.toString()}'`);
        }
      }
...
function parseProgramArgSync(launchArgs: DebugConfiguration): { program: string; dirname: string; programIsDirectory: boolean } {
...
export const appendToDebugConsole = (msg: string) => {
  ...
  qv.debug.activeDebugConsole.append(msg);
};

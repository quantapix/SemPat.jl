...
import { getCurrentGoPath, getModuleCache, getTempFilePath, getWorkspaceFolderPath, handleDiagnosticErrors, ICheckResult, runTool } from './util';
...
export function buildCode(buildWorkspace?: boolean) {
  ...
  if (!buildWorkspace) {
    if (!editor) {
      qv.window.showInformationMessage('No editor is active, cannot find current package to build');
      return;
    }
    if (editor.document.languageId !== 'go') {
      qv.window.showInformationMessage('File in the active editor is not a Go file, cannot find current package to build');
      return;
    }
  }
 ...
 */
export async function goBuild(fileUri: qv.Uri, isMod: boolean, goConfig: qv.WorkspaceConfiguration, buildWorkspace?: boolean): Promise<ICheckResult[]> {
  epoch++;
  ...
  if (buildWorkspace && currentWorkspace && !isTestFile) {
    ...
    return getNonVendorPackages(currentWorkspace).then((pkgs) => {
      running = true;
      return runTool(buildArgs.concat(Array.from(pkgs.keys())), currentWorkspace, 'error', true, null, buildEnv, true, tokenSource.token).then((v) => {
        updateRunning();
        return v;
      });
    });
  ...
  running = true;
  return runTool(buildArgs.concat('-o', tmpPath, importPath), cwd, 'error', true, null, buildEnv, true, tokenSource.token).then((v) => {
    updateRunning();
    return v;
  });
}

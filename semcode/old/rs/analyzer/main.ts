...
async function tryActivate(context: qv.ExtensionContext) {
  ...
  const workspaceFolder = qv.workspace.workspaceFolders?.[0];
...
async function bootstrapExtension(config: Config, state: PersistentState): Promise<void> {
  ...
  if (config.channel === 'stable') {
    if (config.package.releaseTag === NIGHTLY_TAG) {
      void qv.window.showWarningMessage(`You are running a nightly version of rust-analyzer extension. ` + `To switch to stable, uninstall the extension and re-install it from the marketplace`);
    }
    return;
  }
  ...
  const userResponse = await qv.window.showInformationMessage('New version of rust-analyzer (nightly) is available (requires reload).', 'Update');
  ...
  await qv.commands.executeCommand('workbench.extensions.installExtension', qv.Uri.file(dest));

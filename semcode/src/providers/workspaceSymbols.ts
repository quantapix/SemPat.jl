...
class WorkspaceSymbolProvider implements vsc.WorkspaceSymbolProvider {
  ...
  private async toOpenedFiledPath(d: vsc.TextDocument) {
    if (d.uri.scheme === fileSchemes.git) {
      try {
  ...
  private toSymbolInformation(i: Proto.NavtoItem) {
    const l = WorkspaceSymbolProvider.getLabel(i);
  ...
  private static getLabel(i: Proto.NavtoItem) {
    const n = i.name;
...
export function register(c: ServiceClient, ids: readonly string[]) {
  return vsc.languages.registerWorkspaceSymbolProvider(new WorkspaceSymbolProvider(c, ids));
}

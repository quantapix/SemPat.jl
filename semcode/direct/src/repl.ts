...
function get_editor(): string {
  ...
  if (is_remote_env()) {
    if (qv.env.appName === 'Code - OSS') return 'code-server';
    else {
...
async function startREPL(preserveFocus: boolean, showTerminal: boolean = true) {
  if (g_terminal === null) {
    ...
    if (pkgServer.length !== 0) env['JULIA_PKG_SERVER'] = pkgServer;
    ...
    if (pkgenvpath === null) {
      ...
      if (qv.workspace.getConfig('julia').get('useCustomSysimage') && env_file_paths.sysimage_path && env_file_paths.project_toml_path && env_file_paths.manifest_toml_path) {
        ...
        if (date_sysimage.mtime > date_manifest.mtime) sysImageArgs = ['-J', env_file_paths.sysimage_path];
        else {
...
function killREPL() {
  if (g_terminal) g_terminal.dispose();
}
...
async function evaluateBlockOrSelection(shouldMove: boolean = false) {
  ...
  for (const selection of selections) {
    ...
    if (shouldMove && nextBlock && selection.isEmpty && editor.selections.length === 1 && editor.selection === selection) validateMoveAndReveal(editor, nextBlock, nextBlock);
...
async function evaluate(editor: qv.TextEditor, range: qv.Range, text: string, module: string) {
  ...
  if (resultType !== 'REPL') r = results.addResult(editor, range, ' ⟳ ', '');
...
async function executeCodeCopyPaste(text: string, individualLine: boolean) {
  ...
  text = lines.join('\n');
  if (individualLine || process.platform === 'win32') g_terminal.sendText(text + '\n', false);
  else {
...
function signalInterrupt() {
  try {
    if (process.platform !== 'win32') g_terminal.processId.then((pid) => process.kill(pid, 'SIGINT'));
    else {
...
async function getDirUriFsPath(uri: qv.Uri | undefined) {
  if (!uri) {
    const ed = qv.window.activeTextEditor;
    if (ed && ed.document && ed.document.uri) uri = ed.document.uri;
  }
...
export function activate(context: qv.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    qv.window.onDidChangeActiveTerminal((terminal) => {
      if (terminal === g_terminal) setContext('isJuliaREPL', true);
      else {
        setContext('isJuliaREPL', false);
      }
    }),
    qv.window.onDidCloseTerminal((terminal) => {
      if (terminal === g_terminal) g_terminal = null;
    }),

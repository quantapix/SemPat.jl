...
const quickPickButtons = [{ iconPath: new qv.ThemeIcon('save'), tooltip: 'Save as a launch.json configurtation.' }];
...
export async function selectRunnable(ctx: Ctx, prevRunnable?: RunnableQuickPick, debuggeeOnly = false, showButtons: boolean = true): Promise<RunnableQuickPick | undefined> {
  ...
  return await new Promise((resolve) => {
    ...
    const quickPick = qv.window.createQuickPick<RunnableQuickPick>();
...
export class RunnableQuickPick implements qv.QuickPickItem {
  public label: string;
...
export async function createTask(runnable: ra.Runnable, config: Config): Promise<qv.Task> {
  ...
  const target = qv.workspace.workspaceFolders![0]; // safe, see main activate()

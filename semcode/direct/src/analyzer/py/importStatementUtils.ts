...
import { ImportAsNode, ImportFromAsNode, ImportFromNode, ImportNode, ModuleNameNode, ModuleNode, ParseNode, ParseNodeType } from '../parser/parseNodes';
  ...
  if (importStatements.orderedImports.length > 0 && invocation >= importStatements.orderedImports[0].node.start) {
    ...
    for (const statement of parseResults.parseTree.statements) {
      ...
      if (stopHere) {
        ...
        insertionPosition = convertOffsetToPosition(statement.start + statement.length, parseResults.tokenizerOutput.lines);

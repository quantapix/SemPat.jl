...
export function resolveAliasDeclaration(importLookup: ImportLookup, declaration: Declaration, resolveLocalNames: boolean): Declaration | undefined {
  ...
  while (true) {
    if (curDeclaration.type !== DeclarationType.Alias) return curDeclaration;
    if (!curDeclaration.symbolName) return curDeclaration;
    if (!resolveLocalNames && curDeclaration.usesLocalName) return curDeclaration;
    let lookupResult: ImportLookupResult | undefined;
    if (curDeclaration.path) lookupResult = importLookup(curDeclaration.path);
    const symbol: Symbol | undefined = lookupResult ? lookupResult.symbolTable.get(curDeclaration.symbolName) : undefined;
    if (!symbol) {
      if (curDeclaration.submoduleFallback) return resolveAliasDeclaration(importLookup, curDeclaration.submoduleFallback, resolveLocalNames);

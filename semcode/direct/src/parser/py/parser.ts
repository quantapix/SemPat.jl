...
import { DedentToken, IdentifierToken, IndentToken, KeywordToken, KeywordType, NumberToken, OperatorToken, OperatorType, StringToken, StringTokenFlags, Token, TokenType } from './tokenizerTypes';
...
export class Parser {
    ...
    switch (this._peekKeywordType()) {
      ...
      case KeywordType.Match: {
        ...
        if (
          ...
          if (operatorToken.operatorType !== OperatorType.Multiply && operatorToken.operatorType !== OperatorType.Subtract) {
            isInvalidMatchToken = true;
          }
        }
  ...
  private _parsePatternAtom(): PatternAtomNode {
    ...
    if (patternCaptureOrValue) {
      if (patternCaptureOrValue.nodeType === ParseNodeType.Error || !this._consumeTokenIfType(TokenType.OpenParenthesis)) {
        return patternCaptureOrValue;
      }

      const args = this._parseClassPatternArgList();

      const classNameExpr = patternCaptureOrValue.nodeType === ParseNodeType.PatternCapture ? patternCaptureOrValue.target : patternCaptureOrValue.expression;
    ...
    if (nextToken.type === TokenType.OpenParenthesis || nextToken.type === TokenType.OpenBracket) {
      ...
      if (patternList.parseError) {
        casePattern = patternList.parseError;
      } else if (patternList.list.length === 1 && !patternList.trailingComma && startToken.type === TokenType.OpenParenthesis) {
        const pattern = patternList.list[0].orPatterns[0];
  ...
  private _parseClassPatternArgList(): PatternClassArgumentNode[] {
    ...
    while (true) {
      ...
      if (nextTokenType === TokenType.CloseParenthesis || nextTokenType === TokenType.NewLine || nextTokenType === TokenType.EndOfStream) {
        break;
      }
  ...
  private _parsePatternLiteral(): PatternLiteralNode | undefined {
    ...
    if (nextToken.type === TokenType.Keyword) {
      ...
      if (keywordToken.keywordType === KeywordType.False || keywordToken.keywordType === KeywordType.True || keywordToken.keywordType === KeywordType.None) {
  ...
  private _parseWhileStatement(): WhileNode {
    ...
    const whileNode = WhileNode.create(whileToken, this._parseTestExpression(/* allowAssignmentExpression */ true), this._parseLoopSuite());
  ...
  private _parseWithStatement(asyncToken?: KeywordToken): WithNode {
    ...
    if (possibleParen.type === TokenType.OpenParenthesis) {
      ...
      this._suppressErrors(() => {
        ...
        if (this._peekToken().type === TokenType.CloseParenthesis && this._peekToken(1).type === TokenType.Colon) {
  ...
  private _parseDecorator(): DecoratorNode {
    ...
    if (this._getLanguageVersion() < PythonVersion.V3_9) {
      ...
      } else if (expression.nodeType === ParseNodeType.Call && this._isNameOrMemberAccessExpression(expression.leftExpression)) {
        isSupportedExpressionForm = true;
      }
  ...
  private _parseComparison(): ExpressionNode {
    ...
    while (true) {
      ...
      if (Tokenizer.isOperatorComparison(this._peekOperatorType())) {
        ...
        if (tokenAfterNot.type === TokenType.Keyword && (tokenAfterNot as KeywordToken).keywordType === KeywordType.In) {
          this._getNextToken();
          this._getNextToken();
  ...
  private _parseAtomExpression(): ExpressionNode {
    ...
    while (true) {
      ...
      if (this._consumeTokenIfType(TokenType.OpenParenthesis)) {
        ...
        const indexNode = IndexNode.create(atomExpression, subscriptList.list, subscriptList.trailingComma, closingToken);
  ...
  private _parseVariableTypeAnnotationComment(): ExpressionNode | undefined {
    ...
    const parseResults = parser.parseTextExpression(this._fileContents!, stringToken.start, stringToken.length, this._parseOptions, ParseTextMode.VariableAnnotation);
  ...
  private _parseFunctionTypeAnnotationComment(stringToken: StringToken, functionNode: FunctionNode): void {
    ...
    const stringListNode = StringListNode.create([stringNode]);
    const parser = new Parser();
    const parseResults = parser.parseTextExpression(this._fileContents!, stringToken.start, stringToken.length, this._parseOptions, ParseTextMode.FunctionAnnotation);
  ...
  private _parseFormatStringSegment(stringToken: StringToken, segment: StringTokenUtils.FormatStringSegment, segmentOffset: number, segmentLength: number) {
  ...
  private _parseFormatString(stringToken: StringToken): FormatStringNode {
    ...
    return FormatStringNode.create(stringToken, unescapedResult.value, unescapedResult.unescapeErrors.length > 0, formatExpressions);
  }
    ...
    while (segmentExprLength < segmentValue.length) {
      ...
      if (curChar === '=') {
        ...
        if (curChar === ':') {
          ...
          if (segmentExprLength + 2 < segmentValue.length && segmentValue[segmentExprLength + 1] === curChar && segmentValue[segmentExprLength + 2] === curChar) {
  ...
  private _addError(message: string, range: TextRange) {
    ...
    if (!this._areErrorsSuppressed) {
      this._diagSink.addError(message, convertOffsetsToRange(range.start, range.start + range.length, this._tokenizerOutput!.lines));
    }
  }
}

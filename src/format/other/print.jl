function format_check(io::IOBuffer, fst::FST, s::Formatter)
...
function print_leaf(io::IOBuffer, fst::FST, s::Formatter)
    if fst.typ === NOTCODE
...
function print_tree(io::IOBuffer, fst::FST, s::Formatter)
    notcode_indent = -1
    if fst.typ === JLParser.BinaryOpCall ||
       fst.typ === JLParser.ConditionalOpCall ||
       fst.typ === JLParser.ModuleH
    ...
    for (i, n) in enumerate(nodes)
        if n.typ === NOTCODE
            ...
            elseif i + 1 < length(nodes) && (
                nodes[i+2].typ === JLParser.Block || nodes[i+2].typ === JLParser.Begin
            )
        ...
        if n.typ === NEWLINE && s.on && i < length(nodes)
            if is_closer(nodes[i+1]) ||
               nodes[i+1].typ === JLParser.Block ||
               nodes[i+1].typ === JLParser.Begin
...
function print_stringh(io::IOBuffer, fst::FST, s::Formatter)
    # The indent of StringH is set to the the off
...
function print_notcode(io::IOBuffer, fst::FST, s::Formatter; fmttag = false)
    s.on || return
...
function print_inlinecomment(io::IOBuffer, fst::FST, s::Formatter)
    s.on || return

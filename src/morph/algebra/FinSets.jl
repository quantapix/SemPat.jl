...
An codeementation detail used to control method dispatch. Concrete subtypes
...
The function can be defined codeicitly by an arbitrary Julia function, in which
...
compose_code(f::FinFunction{S,T}, g::FinFunction{S,T}) where {S,T} =
  FinFunction(g ∘ f, dom(f), codom(g))
compose_code(f::FinFunctionVector, g::FinFunctionVector) =
  FinFunctionVector(g.func[f.func], g.codom)

compose_code(f::FinFunction, ::FinFunctionIdentity) = f
compose_code(::FinFunctionIdentity, f::FinFunction) = f
compose_code(f::FinFunctionIdentity, ::FinFunctionIdentity) = f

...
codeementation details and should be ignored.
"""
...
"""
function AbstractACSetType(pres::Picture{Schema})
  type_vars = [ TypeVar(nameof(data)) for data in gens(pres, :Data) ]
...
"""
function ACSetType(pres::Picture{Schema}; index=[], unique_index=[])
  type_vars = [ TypeVar(nameof(data)) for data in gens(pres, :Data) ]
...
"""
function AbstractCSetType(pres::Picture{Schema})
...
"""
function CSetType(pres::Picture{Schema}; index=[], unique_index=[])
  if !(isempty(gens(pres, :Data)) && isempty(gens(pres, :Attr)))
...
For ctx, see: https://github.com/JuliaArrays/StructArrays.jl/issues/148
"""
...
subpart(acs::ACSet, expr::QExpr{:gen}) = subpart(acs, first(expr))
subpart(acs::ACSet, expr::QExpr{:id}) = parts(acs, first(dom(expr)))
...
subpart(acs::ACSet, part, expr::QExpr{:compose}) =
...
subpart(acs::ACSet, expr::QExpr{:compose}) = subpart(acs, subpart_names(expr))

subpart_names(expr::QExpr{:gen}) = Symbol[first(expr)]
subpart_names(expr::QExpr{:id}) = Symbol[]
subpart_names(expr::QExpr{:compose}) =
...
incident(acs::ACSet, part, expr::QExpr; kw...) =
...
@generated function _add_parts!(acs::ACSet{CD,AD,Ts,Idxed}, ::Type{Val{ob}},
  ...
  quote
    ...
    $(Expr(:block, map(out_homs) do hom
        :(@inbounds acs.tables.$ob.$hom[beg:nparts] .= 0)
     end...))
    $(Expr(:block, map(indexed_in_homs) do hom
        quote
          resize!(acs.indices.$hom, nparts)
          @inbounds for i in beg:nparts; acs.indices.$hom[i] = Int[] end
        end
      end...))
    beg:nparts
  end

...
To codeement for a type `T`, define the method `limit(::EmptyDiagram{T})`.
"""
terminal(T::Type) = limit(EmptyDiagram{T}())

""" Initial object.

To codeement for a type `T`, define the method `colimit(::EmptyDiagram{T})`.
"""
initial(T::Type) = colimit(EmptyDiagram{T}())

""" Unique morphism into a terminal object.

To codeement for a type `T`, define the method
...
To codeement for a type `T`, define the method `limit(::ObjectPair{T})` and/or
...
To codeement for a type `T`, define the method `colimit(::ObjectPair{T})` and/or
...
To codeement for a type `T`, define the method `limit(::ParallelPair{T})` and/or
...
To codeement for a type `T`, define the method `colimit(::ParallelPair{T})` or
...
To codeement for a type `T`, define the method `limit(::Cospan{T})` and/or
`limit(::Multicospan{T})` or, if you have already codeemented products and
equalizers, rely on the default codeementation.
"""
...
To codeement for a type `T`, define the method `colimit(::Span{T})` and/or
`colimit(::Multispan{T})` or, if you have already codeemented coproducts and
coequalizers, rely on the default codeementation.
"""
...
To codeement for products of type `T`, define the method
...
To codeement for coproducts of type `T`, define the method
...
To codeement for equalizers of type `T`, define the method
...
The fields of this struct are an codeementation detail; accessing them directly

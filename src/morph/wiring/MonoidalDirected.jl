...
export Ports, Junction, PortOp, BoxOp, functor, permute,
  codeicit_mcopy, codeicit_mmerge, junctioned_mcopy, junctioned_mmerge,
...
Box(value, ins::Ports, outs::Ports) =
  Box(value, collect(ins), collect(outs))
Box(ins::Ports, outs::Ports) = Box(collect(ins), collect(outs))

WiringDiagram(value, ins::Ports{T}, outs::Ports{T}) where T =
  WiringDiagram{T}(value, collect(ins), collect(outs))

WiringDiagram(ins::Ports{T}, outs::Ports{T}) where T =
  WiringDiagram{T}(collect(ins), collect(outs))
...
function functor(d::WiringDiagram, f_ports, f_box; contravariant::Bool=false, kw...)
  functor_code = contravariant ? contravariant_functor : covariant_functor
  functor_code(d, f_ports, f_box; kw...)
end
...
"""
function codeicit_mcopy(A::Ports, n::Int)
...
"""
function codeicit_mmerge(A::Ports, n::Int)
...
mcopy(A::Ports{Any}, n::Int) = codeicit_mcopy(A, n)
mmerge(A::Ports{Any}, n::Int) = codeicit_mmerge(A, n)

# Default codeementation for biased (co)diagonals fall backs to unbiased
...
mcopy(A::Ports{MonoidalCategoryWithDiagonals}, n::Int) = codeicit_mcopy(A, n)

mcopy(A::Ports{CartesianCategory}, n::Int) = codeicit_mcopy(A, n)

# Cocartesian category
#---------------------

mmerge(A::Ports{MonoidalCategoryWithCodiagonals}, n::Int) = codeicit_mmerge(A, n)

mmerge(A::Ports{CocartesianCategory}, n::Int) = codeicit_mmerge(A, n)
...
mcopy(A::Ports{BiproductCategory}, n::Int) = codeicit_mcopy(A, n)
mmerge(A::Ports{BiproductCategory}, n::Int) = codeicit_mmerge(A, n)
...
struct Junction{Op,Value} <: AbstractBox
  ...
  input_ports::Vector
  output_ports::Vector
  Junction{Op}(value::Value, ins::Vector, outs::Vector) where {Op,Value} =
    new{Op,Value}(value, ins, outs)
end

Junction(args...) = Junction{nothing}(args...)
Junction{Op}(value, nins::Int, nouts::Int) where Op =
  Junction{Op}(value, repeat([value], nins), repeat([value], nouts))
...
junction_cups(A::Ports, ins::Ports; op=nothing) =
  junction_cups(Junction{op}, A, ins)

function junction_cups(make_junction, A::Ports, ins::Ports)
  @assert length(ins) == 2*length(A)
  f = WiringDiagram(ins, munit(typeof(ins)))
  m, ports = length(A), collect(ins)
  for (i, value) in enumerate(A)
...
junction_caps(A::Ports, outs::Ports; op=nothing) =
  junction_caps(Junction{op}, A, outs)

function junction_caps(make_junction, A::Ports, outs::Ports)
  @assert length(outs) == 2*length(A)
  f = WiringDiagram(munit(typeof(outs)), outs)
  m, ports = length(A), collect(outs)
  for (i, value) in enumerate(A)
...
Transforms from the codeicit to the explicit representation of diagonals and
...
Transforms from the explicit to the codeicit representation of diagonals and
...
function rem_junctions(d::WiringDiagram; op=nothing)
  ...
  junction_diagrams = map(junction_ids) do v
    ...
    ins, outs = input_ports(junction), output_ports(junction)
    wiring = WiringDiagram(ins, outs)
    add_wires!(wiring, (input_id(wiring), src) => (output_id(wiring), tgt)
                        for src in eachindex(ins), tgt in eachindex(outs))
    wiring
  end

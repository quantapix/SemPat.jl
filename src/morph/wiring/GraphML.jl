...
export read_graphml, parse_graphml, write_graphml, make_graphml,
...
function write_graphml(diagram::WiringDiagram, filename::String)
  LightXML.save_file(make_graphml(diagram), filename)
end

""" Generate GraphML representing a wiring diagram.
"""
function make_graphml(diagram::WiringDiagram)::XMLDocument
  # Create XML document.
  xdoc = XMLDocument()
  ...
  state = WriteState()
  make_graphml_node(state, xgraph, diagram, Int[])
...
function make_graphml_node(
  ...
  make_graphml_ports(state, xnode, diagram)
  
  xsubgraph = new_child(xnode, "graph")
  set_attribute(xsubgraph, "id", string(box_id(path), ":graph"))
  make_graphml_data(state, xsubgraph, "graph", diagram.value)
  
  # Add node elements for boxes.
  for v in box_ids(diagram)
    make_graphml_node(state, xsubgraph, box(diagram, v), [path; v])
  end
  ...
  for (i, wire) in enumerate(wires(diagram))
    ...
    make_graphml_data(state, xedge, "edge", wire.value)
  end
end
...
function make_graphml_node(
  ...
  xnode = new_child(xgraph, "node")
  set_attribute(xnode, "id", box_id(path))
  make_graphml_data(state, xnode, "node", box.value)
  make_graphml_ports(state, xnode, box)
end

function make_graphml_ports(state::WriteState, xnode::XMLElement, box::AbstractBox)
  # Write input ports.
  for (i, port) in enumerate(input_ports(box))
    xport = new_child(xnode, "port")
    set_attribute(xport, "name", port_name(InputPort, i))
    make_graphml_data(state, xport, "port", Dict("portkind" => "input"))
    make_graphml_data(state, xport, "port", port)
  end
  # Write output ports.
  for (i, port) in enumerate(output_ports(box))
    xport = new_child(xnode, "port")
    set_attribute(xport, "name", port_name(OutputPort, i))
    make_graphml_data(state, xport, "port", Dict("portkind" => "output"))
    make_graphml_data(state, xport, "port", port)
  end
end

function make_graphml_key(xroot::XMLElement, key::GraphMLKey)
  ...
  if !isnothing(key.default)
    ...
    set_content(xdefault, make_graphml_data_value(something(key.default)))
  end
end

function make_graphml_data(state::WriteState, xelem::XMLElement, scope::String, value)
  ...
  for (attr_name, attr_value) in data
    ...
    key = get!(state.keys, (attr_name, scope)) do
      ...
      id = "d$(nkeys+1)"
      attr_type = make_graphml_data_type(typeof(attr_value))
    ...
    xvalue = make_graphml_data_value(attr_value)
    (xvalue isa AbstractString ? set_content : add_child)(xdata, xvalue)
  end
end

make_graphml_data_type(::Type{Bool}) = "boolean"
make_graphml_data_type(::Type{<:Integer}) = "int"
make_graphml_data_type(::Type{<:Real}) = "double"
make_graphml_data_type(::Type{String}) = "string"
make_graphml_data_type(::Type{Symbol}) = "string"
make_graphml_data_type(::Type{Dict{String,T}}) where T = "json"
make_graphml_data_type(::Type{Vector{T}}) where T = "json"

make_graphml_data_value(x::Number) = string(x)
make_graphml_data_value(x::String) = x
make_graphml_data_value(x::Symbol) = string(x)
make_graphml_data_value(x::Dict) = JSON.json(x)
make_graphml_data_value(x::Vector) = JSON.json(x)

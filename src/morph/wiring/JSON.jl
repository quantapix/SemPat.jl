...
export read_json_graph, parse_json_graph, write_json_graph, make_json_graph,
...
function write_json_graph(diagram::WiringDiagram, filename::String;
  ...
  open(filename, "w") do io
    JSON.print(io, make_json_graph(diagram), indent)
  end
end

""" Generate a JSON dict representing a wiring diagram.
"""
function make_json_graph(diagram::WiringDiagram)::AbstractDict
  make_json_box(diagram, Int[])
end

function make_json_box(diagram::WiringDiagram, path::Vector{Int})
  json_object_with_value(diagram.value,
    "id" => box_id(path),
    "ports" => make_json_ports(diagram),
    "children" => [
      make_json_box(box(diagram, v), [path; v]) for v in box_ids(diagram)
    ],
    "edges" => [
...
function make_json_box(box::Box, path::Vector{Int})
  json_object_with_value(box.value,
    "id" => box_id(path),
    "ports" => make_json_ports(box),
  )
end

function make_json_ports(box::AbstractBox)::AbstractArray
  [
    [

""" Convert wiring diagrams to and from syntactic exprs.
...
exprs in the textual syntax: a single morphism may correspond to many
wiring diagrams, and a single wiring diagram may correspond to many syntactic
exprs.

It is trivial to convert a morphism expr to a wiring diagram, but not to
go the other way around.
"""
module WiringDiagramExpressions
export to_ob_expr, to_hom_expr, to_wirings, to_undirected_wirings

using Compat

using ...Syntax, ...Theories, ...Permutations
using ...Syntax: syntax_mod
...
""" Convert a morphism expr into a wiring diagram.
"""
function to_wirings(expr::QExpr, args...)
  T = syntax_mod(expr).theory()
  to_wirings(T, expr, args...)
end
function to_wirings(T::Type, expr::QExpr)
  to_wirings(T, expr, first, first)
end
function to_wirings(T::Type, expr::QExpr, ob_map, hom_map)
  to_wd = expr -> to_wirings(T, expr, ob_map, hom_map)
...
""" Convert a morphism expr into an undirected wiring diagram.

Returns a `HomUWD`, or a UWD with outer ports partitioned domain and codomain.
"""
function to_undirected_wirings(expr::QExpr)
  to_undirected_wirings(expr, Symbol, Symbol, first, first)
end
function to_undirected_wirings(expr::QExpr, Ob::Type, Hom::Type,
                                      ob_map, hom_map)
  to_uwd = expr -> to_undirected_wirings(expr, Ob, Hom, ob_map, hom_map)
...
""" Convert a wiring diagram into a morphism expr.
"""
...
function series_reduction(Ob::Type, Hom::Type, d::WiringDiagram)
  ...
  composites = map(series) do vs
    ...
    foldl(compose_scodeify_id, exprs)
  end
...
function hom_expr_between(Ob::Type, diagram::WiringDiagram, v1::Int, v2::Int)
  ...
  ins, outs = to_ob_exprs(Ob, ins), to_ob_exprs(Ob, outs)
  @assert ins == outs[σ]
  permutation_to_expr(σ, ins)
end

""" Convert a port value into an object expr.
"""
...
""" Compose morphism exprs, eliminating identities.
"""
function compose_scodeify_id(f::QExpr, g::QExpr)
  if head(f) == :id; g
  elseif head(g) == :id; f
  else compose(f,g) end
end

""" Convert a junction node to a morphism expr.
"""
function junction_to_expr(Ob::Type, junction::Junction)
  ...
  else compose_scodeify_id(mmerge_foldl(ob, nin), mcopy_foldl(ob, nout)) end
end

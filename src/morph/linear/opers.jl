...
@instance LinearFunctions{LinearOpDom,LinearOperator} begin
    ...
    swap(V::LinearOpDom, W::LinearOpDom) = opExtension(1:W.N, V.N + W.N) * opRestriction((V.N + 1):(V.N + W.N), V.N + W.N) + opExtension((W.N + 1):(V.N + W.N), V.N + W.N) * opRestriction(1:V.N, V.N + W.N)
    mcopy(V::LinearOpDom) = opExtension(1:V.N, 2 * V.N) + opExtension((V.N + 1):(2 * V.N), 2 * V.N)
    delete(V::LinearOpDom) = opZeros(0, V.N)
    plus(V::LinearOpDom) = opRestriction(1:V.N, 2 * V.N) + opRestriction((V.N + 1):(2 * V.N), 2 * V.N)

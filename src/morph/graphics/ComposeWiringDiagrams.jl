...
""" A Compose ctx together with a given width and height.

We need this type because ctxs have no notion of size or aspect ratio, but
...
function Base.show(io::IO, ::MIME"text/html", pic::ComposePicture)
  ...
  C.draw(C.SVGJS(io, pic.width, pic.height, false, jsmode=jsmode), pic.ctx)
end
function Base.show(io::IO, ::MIME"image/svg+xml", pic::ComposePicture)
  C.draw(C.SVG(io, pic.width, pic.height, false), pic.ctx)
end
...
function layout_to_composejl(diagram::WiringDiagram; props=Dict(), kw...)
  ...
  ctx = layout_to_ctx(diagram, opts)
  if !isnothing(opts.background_color)
    ctx = C.compose(C.ctx(),
      ctx,
      (C.ctx(), C.rectangle(), C.fill(opts.background_color)))
  end
  ComposePicture(ctx, (opts.base_unit .* size(diagram))...)
end

function layout_to_ctx(diagram::WiringDiagram, opts::ComposeOptions)
  ...
  C.compose(C.ctx(units=units, tag=:diagram),
    C.compose(C.ctx(tag=:boxes),
      map(boxes(diagram)) do box
        layout_to_ctx(box, opts)
      end...
    ),
    C.compose(C.ctx(tag=:wires),
      map(wires(diagram)) do wire
        layout_to_ctx(diagram, wire, opts)
      end...
    ),
  )
end

function layout_to_ctx(box::Box, opts::ComposeOptions)
  C.compose(C.ctx(lower_corner(box)..., size(box)...,
                      units=C.UnitBox(), tag=:box),
    opts.box_renderer(box.value, opts)
  )
end

function layout_to_ctx(diagram::WiringDiagram, wire::Wire, opts::ComposeOptions)
...
render_box(::Type{Val{:junction}}, layout::BoxLayout, opts::ComposeOptions) =
  C.compose(C.ctx(), C.circle(), box_props(layout, opts)...)
render_box(::Type{Val{:invisible}}, ::BoxLayout, ::ComposeOptions) = C.ctx()
...
function rounded_rectangle(args...; corner_radius::Compose.Measure=C.mm)
  ...
  r = corner_radius
  C.compose(C.ctx(args...),
    C.compose(C.ctx(order=1),
      # Layer 1: fill.

...
The input `f` can also be a morphism expr, which is converted into a
wiring diagram.
...
function to_graphviz(f::WiringDiagram;
  ...
  for (i, wire) in enumerate(wires(f))
    ...
    # "more specific" type when codeicit conversions are allowed.
...
function graphviz_box(box::AbstractBox, node_id;
  ...
  ins = [ graphviz_port(InputPort, i) for i in 1:nin ]
  outs = [ graphviz_port(OutputPort, i) for i in 1:nout ]
  
  GraphvizBox([node], ins, outs)
end
...
function graphviz_outer_box(f::WiringDiagram;
  ...
  stmts = Graphviz.Statement[]
  nins, nouts = length(input_ports(f)), length(output_ports(f))
  if nins > 0
    push!(stmts, graphviz_outer_ports(input_id(f), InputPort, nins;
      anchor=anchor, orientation=orientation))
  end
  if nouts > 0
    push!(stmts, graphviz_outer_ports(output_id(f), OutputPort, nouts;
      anchor=anchor, orientation=orientation))
  end
  ...
  ins = [ graphviz_port(Port(input_id(f), OutputPort, i)) for i in 1:nins ]
  outs = [ graphviz_port(Port(output_id(f), InputPort, i)) for i in 1:nouts ]
  
  GraphvizBox(stmts, ins, outs)
end
...
- `box_labels=false`: if boolean, whether to label boxes with their number;
   if a sym, name of data attribute for box label
...
- `junction_labels=false`: if boolean, whether to label junctions with their
  number; if a sym, name of data attribute for junction label
- `junction_size="0.075"`: size of junction nodes, in inches
- `codeicit_junctions=false`: whether to represent a junction codeicity as a
...
function to_graphviz_property_graph(d::UndirectedWiringDiagram;
    ...
    junction_size::String="0.075", codeicit_junctions::Bool=false,
    graph_attrs::AbstractDict=Dict(), node_attrs::AbstractDict=Dict(),
  ...
  for j in junctions(d)
    ...
    if codeicit_junctions && length(incident_ports) == 2
...
function graphviz_layout(diagram::WiringDiagram, graph::PropertyGraph)
  ...
  ins = map(enumerate(input_ports(diagram))) do (i, value)
  ...
  outs = map(enumerate(output_ports(diagram))) do (i, value)
  ...
  layout = WiringDiagram(BoxLayout(size=diagram_size), ins, outs)
  ...
  for (i, box) in enumerate(boxes(diagram))
    ...
    @assert shape == :rectangle "Only the rectangle shape is codeemented so far"

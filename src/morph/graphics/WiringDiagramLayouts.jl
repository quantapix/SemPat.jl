""" Backend-agnostic layout of wiring diagrams via morphism exprs.

This module lays out wiring diagrams for visualization, independent of any
specific graphics system. It uses the structure of a morphism expr to
determine the layout. Thus, the first step of the algorithm is to convert the
wiring diagram to a symic expr, using the submodule
`WiringDiagrams.Expressions`. Morphism exprs may also be given directly.
"""
...
"""
@with_kw_noshow mutable struct BoxLayout
  value::Any = nothing
...
"""
@with_kw_noshow struct PortLayout
  value::Any = nothing
...
""" Lay out a wiring diagram or morphism expr for visualization.

If a wiring diagram is given, it is first to converted to a morphism expr.
...
# Morphism expr layout
############################

""" Lay out a morphism expr as a wiring diagram.
"""
...
function layout_pure_wiring(f::HomExpr, opts::LayoutOptions; kw...)
  layout_pure_wiring(to_wirings(f, identity, identity), opts;
...
"""
function layout_box(ins::Vector, outs::Vector, opts::LayoutOptions;
  ...
  layout_box(Val{shape}, ins, outs, opts; style=style, value=value, kw...)
end

function layout_box(::Type{Val{:rectangle}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; shape::Symbol=:rectangle, kw...)
  size = default_box_size(length(ins), length(outs), opts)
  box = Box(BoxLayout(; shape=shape, size=size, kw...),
            layout_linear_ports(InputPort, ins, size, opts),
            layout_linear_ports(OutputPort, outs, size, opts))
  size_to_fit!(singleton_diagram(box), opts)
end

function layout_box(::Type{Val{:circle}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; pad::Bool=true, kw...)
  size = default_box_size(1, 1, opts)
  radius = first(size) / 2
  box = Box(BoxLayout(; shape=:circle, size=size, kw...),
            layout_circular_ports(InputPort, ins, radius, opts; pad=pad),
            layout_circular_ports(OutputPort, outs, radius, opts; pad=pad))
  size_to_fit!(singleton_diagram(box), opts)
end

function layout_box(::Type{Val{:ellipse}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; pad::Bool=true, kw...)
  size = default_box_size(length(ins), length(outs), opts)
  radii = size / 2
  box = Box(BoxLayout(; shape=:ellipse, size=size, kw...),
            layout_elliptical_ports(InputPort, ins, radii, opts; pad=pad),
            layout_elliptical_ports(OutputPort, outs, radii, opts; pad=pad))
  size_to_fit!(singleton_diagram(box), opts)
end

function layout_box(::Type{Val{:triangle}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; kw...)
  @assert length(ins) <= 1 "Cannot use triangle shape with multiple ins"
  layout_box(Val{:rectangle}, ins, outs, opts; shape=:triangle, kw...)
end
function layout_box(::Type{Val{:invtriangle}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; kw...)
  @assert length(outs) <= 1 "Cannot use invtriangle shape with multiple outs"
  layout_box(Val{:rectangle}, ins, outs, opts; shape=:invtriangle, kw...)
end
...
function layout_box(::Type{Val{:trapezium}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; kw...)
  layout_box(Val{:rectangle}, ins, outs, opts; shape=:trapezium, kw...)
end
function layout_box(::Type{Val{:invtrapezium}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; kw...)
  layout_box(Val{:rectangle}, ins, outs, opts; shape=:invtrapezium, kw...)
end

function layout_box(::Type{Val{:junction}}, ins::Vector, outs::Vector,
                    opts::LayoutOptions; visible::Bool=true, pad::Bool=true, kw...)
  nin, nout = length(ins), length(outs)
  shape, radius = visible ? (:junction, opts.junction_size) : (:invisible, 0)
  size = 2*SVector(radius, radius)
  box = Box(BoxLayout(; shape=shape, size=size, kw...),
            layout_circular_ports(InputPort, ins, radius, opts;
                                  pad=pad, label_wires = nin == 1 || nout == 0),
            layout_circular_ports(OutputPort, outs, radius, opts;
...
function layout_outer_ports(diagram::WiringDiagram, opts::LayoutOptions,
                            ::Type{Val{:isotonic}})
  ins, outs = input_ports(diagram), output_ports(diagram)
  ...
  # full Euclidean plan.
  ys = [ Float64[] for i in eachindex(ins) ]
  ...
  ys = [ Float64[] for i in eachindex(outs) ]
  ...
  (layout_linear_ports(InputPort, ins, in_coords, diagram_size, opts),
   layout_linear_ports(OutputPort, outs, out_coords, diagram_size, opts))
end
...
function layout_pure_wiring(diagram::WiringDiagram, opts::LayoutOptions;
  ...
  ins, outs = input_ports(diagram), output_ports(diagram)
  size = minimum_diagram_size(length(ins), length(outs), opts)
  result = WiringDiagram(BoxLayout(size=size), ins, outs)
  if anchor_wires
...
function diagram_element_label(::MIME"text/latex", expr::QExpr)

...
function consistent_env_var(v::TVar, env::Env, tds::TyDeclCol, state::ST_State)
  if state.in_consistency_check; return true
  end
  ...
  if is_notfound_marker(vi); lj_error(string("Internal: consistent_env on missing var.  v=", v, "\nenv= ", env_work, "\n"))
  end
  ve = env_work.curr[vi]
  if isa(ve.tag, TagLeft); return true
  end
...
function consistent_env(env::Env, tds::TyDeclCol, state::ST_State)
  ...
  vl = lj_flatten(vcat(map(ee -> ee.var, Base.filter(ee -> isa(ee, VEnv) && isa(ee.tag, TagRight), vcat(env.curr, env.past)))))
  return all(v -> consistent_env_var(v, env, tds, state), vl)
end

sr_consistent_env(sr, tds, state) = sr.sub == true && !(consistent_env(sr.env, tds, state)) ? SR(false, sr.env, state.stats) : sr
...
function lj_subtype(t1::ASTBase, t2::ASTBase, ::TyDeclCol, env::Env, state::ST_State)
    ...
    debug_stat_subtype_result("ASTBase, ASTBase", 
        sr.sub ? unlj("Refl($(typeof(t1)))") : unlj("($(typeof(t1)), $(typeof(t2)))"),
        state, sr)
    return sr
end
...
function save_occ(env::Env)
  occs_dict = Dict()
  for ee in env.curr
    if isa(ee, VEnv) && isa(ee.tag, TagRight); occs_dict[ee.var] = (ee.occ.cov, ee.occ.inv)
    end
  end
  return occs_dict
end
